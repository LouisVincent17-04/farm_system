...
ob_start(); // Buffer output to prevent HTML leaking into AJAX
error_reporting(E_ALL);
ini_set('display_errors', 0); // Disable UI errors for cleaner JSON
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['confirm_sale'])) {
    try {
        ...
        // Calculate Price Per KG (for record keeping)
        ...
        $stmt->execute([
            $animal_id, $_POST['customer_name'], $weight, $price_per_kg, $final_price,
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-select, .form-input, .form-textarea {
            ...
            color: white; border-radius: 8px; font-size: 0.95rem; transition: all 0.2s;
        }
        ...
        .btn-confirm {
            width: 100%; padding: 1rem; background: linear-gradient(135deg, #fbbf24, #d97706);
            color: #0f172a; font-weight: 800; border: none; border-radius: 8px; cursor: pointer;
            margin-top: 1rem; font-size: 1rem; text-transform: uppercase; letter-spacing: 1px;
...
<div class="container">
    ...
    <div class="main-grid">
        ...
        <div class="glass-card">
            <h3 style="color: #fbbf24; margin-bottom: 1.5rem; display: flex; align-items: center; gap: 8px;">
                🔍 Find Animal
            </h3>
            ...
            <div class="form-group">
                ...
                <select id="building_id" class="form-select" onchange="loadPens()" disabled>
            ...
            <div class="form-group">
                <label>4. Select Tag to Sell</label>
                <select id="animal_id_select" class="form-select" onchange="fetchAnimalCosts(this.value)" disabled>
            ...
            <div class="form-group" style="display: flex; gap: 8px;">
                <input type="text" id="search_tag_input" class="form-input" placeholder="Enter Tag No (e.g., A001)">
                <button type="button" class="btn-search" onclick="searchByTag()">GO</button>
            </div>
        ...
        <div class="right-column-wrapper">
            ...
            <div id="dashboard_view">
                <div class="stats-row">
                    <div class="stat-box">
                        <div class="stat-label">Gross Profit Today</div>
                        <div class="stat-num" style="color: #34d399;">₱<?= number_format($stats['total_profit'], 2) ?></div>
                    </div>
                    <div class="stat-box">
                ...
                <div class="glass-card">
                    ...
                    <?php if(empty($recent_sales)): ?>
                        <p style="color: #64748b; text-align: center; padding: 2rem;">No recent sales found.</p>
                    <?php else: ?>
                        <table class="recent-table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td style="color: #fbbf24; font-weight: bold;"><?= $sale['TAG_NO'] ?></td>
                                    <td><?= htmlspecialchars($sale['customer_name']) ?></td>
                                    <td><?= date('M d, H:i', strtotime($sale['sale_date'])) ?></td>
                                    <td style="text-align: right; font-family: monospace;">₱<?= number_format($sale['final_sale_price'], 2) ?></td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
            ...
            <div id="salesFormContainer" class="glass-card">
                <form method="POST">
                    ...
                    <div class="section-header">
                        <div>
                            <div style="color: #94a3b8; font-size: 0.8rem;">PROCESSING SALE FOR</div>
                            <div class="tag-badge" id="disp_tag_header"></div>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="customer_name" class="form-input" required placeholder="Who is buying?">
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                        <div class="form-group">
                            <label>Animal Weight (kg) <small style="color: #64748b;">(Read Only)</small></label>
                            <input type="number" step="0.01" name="weight_at_sale" id="weight" class="form-input" readonly>
                        </div>

                        <div class="form-group">
                            <div id="div_total_price">
                                <label style="color: #fbbf24;">Final Sale Price (₱)</label>
                                <input type="number" step="0.01" name="final_sale_price" id="final_sale_price" class="form-input" 
                                       required placeholder="0.00" oninput="calculateProfit()" style="border-color: #fbbf24;">
                            </div>
                    ...
                    <div style="background: rgba(15,23,42,0.5); padding: 1rem; border-radius: 8px; margin: 1.5rem 0;">
                        ...
                        <input type="hidden" name="cost_acquisition" id="c_acq"><input type="hidden" name="cost_feed" id="c_feed">
                        <input type="hidden" name="cost_medication" id="c_med"><input type="hidden" name="cost_vaccination" id="c_vac">
                        <input type="hidden" name="cost_vitamins" id="c_vit"><input type="hidden" name="cost_checkup" id="c_chk">
                        ...
                        <div class="cost-row" style="align-items: center;">
                            <span>Overhead (Adjustable)</span> 
                            <input type="number" name="cost_overhead" id="c_ovr" value="0.00" step="0.01" oninput="calculateProfit()"
                                   style="width: 80px; padding: 2px; background: transparent; border: 1px solid #475569; color: #fbbf24; text-align: right;">
                        </div>
                        <div class="cost-row total-row">
                            <span>TOTAL NET WORTH</span> <span id="disp_total_cost" style="color: #f472b6;">₱0.00</span>
                        </div>
                    </div>

                    <div class="form-group">
                        <label>Notes</label>
                        <textarea name="notes" class="form-textarea" placeholder="Optional details..."></textarea>
                    </div>
...
<script>
    ...
    async function loadBuildings() {
        ...
        if(locId) {
            ...
            fillSelect(bSelect, data, 'BUILDING_ID', 'BUILDING_NAME');
    ...
    async function loadPens() {
        ...
        resetSelects(['pen_id', 'animal_id_select']);
        if(bId) {
            const data = await fetchData(`?action=get_pens&building_id=${bId}`);
            fillSelect(pSelect, data, 'PEN_ID', 'PEN_NAME');
            pSelect.disabled = false;
        }
    }
    async function loadAnimals() {
        const pId = document.getElementById('pen_id').value;
        const aSelect = document.getElementById('animal_id_select');
        resetSelects(['animal_id_select']);
        if(pId) {
            const data = await fetchData(`?action=get_animals&pen_id=${pId}`);
            fillSelect(aSelect, data, 'ANIMAL_ID', 'TAG_NO');
    ...
    function fillSelect(el, data, valKey, txtKey) {
        el.innerHTML = '<option value="">-- Select --</option>';
        data.forEach(i => el.innerHTML += `<option value="${i[valKey]}">${i[txtKey]}</option>`);
    }

    // --- SEARCH ---
    async function searchByTag() {
        const tag = document.getElementById('search_tag_input').value;
        const err = document.getElementById('search_error');
        err.innerText = "";
        if(!tag) return;
        const res = await fetchData(`?action=search_animal_by_tag&tag=${encodeURIComponent(tag)}`);
        if(res.success) fetchAnimalCosts(res.animal_id);
        else err.innerText = "Tag not found or unavailable.";
    }
    ...
    async function fetchAnimalCosts(id) {
        ...
        if(res.success) {
            ...
            const healthTotal = parseFloat(c.medication) + parseFloat(c.vaccination) + parseFloat(c.vitamins) + parseFloat(c.checkup);
            document.getElementById('disp_health').innerText = fmt(healthTotal);
    ...
    function calculateProfit() {
        ...
        const pDisp = document.getElementById('profitDisplay');
        const label = document.querySelector('#profitBox div:first-child');

        pDisp.innerText = "₱" + fmt(profit);
        pBox.className = profit >= 0 ? "profit-box profit-pos" : "profit-box profit-neg";

        // Update Label context
        const mode = document.querySelector('input[name="price_mode"]:checked').value;
        if(mode === 'per_kg') {
            const pKg = document.getElementById('price_per_kg_input').value;
            label.innerText = `Profit (Based on ₱${pKg}/kg)`;
        } else {
            label.innerText = "ESTIMATED PROFIT";
        }
    }
