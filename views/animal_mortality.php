...
if (isset($_GET['action'])) {
    ...
    try {
        ...
        if ($action === 'get_animals' && isset($_GET['pen_id'])) {
            $stmt = $conn->prepare("SELECT ANIMAL_ID, TAG_NO FROM animal_records WHERE PEN_ID = ? AND IS_ACTIVE = 1 AND CURRENT_STATUS = 'Active' ORDER BY TAG_NO");
            $stmt->execute([$_GET['pen_id']]);
            echo json_encode($stmt->fetchAll(PDO::FETCH_ASSOC)); exit;
        }
        if ($action === 'search_animal_by_tag' && isset($_GET['tag'])) {
            $tag = trim($_GET['tag']);
            $stmt = $conn->prepare("SELECT ANIMAL_ID FROM animal_records WHERE TAG_NO = ? AND IS_ACTIVE = 1 AND CURRENT_STATUS = 'Active'");
            $stmt->execute([$tag]);
...
// 2. PROCESS MORTALITY
// =========================================================
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['confirm_mortality'])) {
    try {
        $conn->beginTransaction();

        $animal_id = $_POST['animal_id'];
        
        // Combine Reason (Dropdown) + Details (Textarea)
        $reason = $_POST['loss_reason'];
        $details = trim($_POST['notes']);
        $final_notes = $reason . ($details ? " - " . $details : "");
        ...
        $recovered_cost = floatval($_POST['recovered_cost']);
        $weight = floatval($_POST['weight_at_death']);
        
        // Profit (Loss)
        $profit = $recovered_cost - $net_worth; 
        
        $current_user = $_SESSION['user_id'] ?? 0; 
        $mortality_date = $_POST['mortality_date'];
        
        $customer_name = !empty($_POST['customer_name']) ? $_POST['customer_name'] : 'N/A';

        // Insert Record
        $sql = "INSERT INTO animal_sales 
            (animal_id, sale_date, customer_name, weight_at_sale, price_per_kg, final_sale_price, 
             cost_acquisition, cost_feed_total, cost_medication_total, cost_vaccination_total, 
             cost_checkup_total, cost_vitamins_total, cost_overhead, total_net_worth, gross_profit, notes, created_by, transaction_type) 
            VALUES 
            (?, ?, ?, ?, 0, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)";
        
        $stmt = $conn->prepare($sql);
        $stmt->execute([
            $animal_id, $mortality_date, $customer_name, $weight, $recovered_cost,
            $_POST['cost_acquisition'], $_POST['cost_feed'], $_POST['cost_medication'],
            $_POST['cost_vaccination'], $_POST['cost_checkup'], $_POST['cost_vitamins'],
            $_POST['cost_overhead'], $net_worth, $profit, $final_notes, 
            $current_user
        ]);

        // Archive Animal
        $updateStmt = $conn->prepare("UPDATE animal_records SET CURRENT_STATUS = 'Deceased', IS_ACTIVE = 0, CURRENT_ACTUAL_WEIGHT = ? WHERE ANIMAL_ID = ?");
        ...
        $_SESSION['flash_message'] = "<div class='alert alert-warning'>Error: " . $e->getMessage() . "</div>";
        header("Location: " . $_SERVER['PHP_SELF']); 
        exit();
    }
}
...
// Fetch recent mortality records
$recentStmt = $conn->query("SELECT s.*, a.TAG_NO FROM animal_sales s JOIN animal_records a ON s.animal_id = a.ANIMAL_ID WHERE s.transaction_type = 0 ORDER BY s.sale_date DESC LIMIT 5");
$recent_sales = $recentStmt->fetchAll(PDO::FETCH_ASSOC);

// Stats
$statsStmt = $conn->query("SELECT COUNT(*) as total_deaths, COALESCE(SUM(total_net_worth), 0) as total_lost_investment, COALESCE(SUM(gross_profit), 0) as net_impact FROM animal_sales WHERE transaction_type = 0 AND DATE(sale_date) = CURDATE()");
...
<head>
    ...
    <title>Mortality Record - FarmPro</title>
    <style>
        /* --- STYLES --- */
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .page-title { font-size: 2rem; font-weight: 800; margin-bottom: 0.5rem; background: linear-gradient(135deg, #ef4444, #b91c1c); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        ...
        .glass-card { 
            ...
            max-height: calc(100vh - 4rem); overflow-y: auto; position: sticky; top: 2rem;
            scrollbar-width: thin; scrollbar-color: #ef4444 #0f172a;
        }
        .glass-card::-webkit-scrollbar { width: 6px; }
        .glass-card::-webkit-scrollbar-track { background: #0f172a; border-radius: 3px; }
        .glass-card::-webkit-scrollbar-thumb { background-color: #ef4444; border-radius: 3px; }
        ...
        .form-input:focus, .form-select:focus { border-color: #ef4444; outline: none; box-shadow: 0 0 0 2px rgba(239, 68, 68, 0.2); }
        ...
        .stat-num { font-size: 1.8rem; font-weight: 800; margin-top: 5px; }
        ...
        .tag-badge { background: #ef4444; color: #fff; padding: 4px 12px; border-radius: 20px; font-weight: 800; font-size: 1.2rem; }
        ...
        .btn-confirm { width: 100%; padding: 1rem; background: linear-gradient(135deg, #ef4444, #b91c1c); color: #fff; font-weight: 800; border: none; border-radius: 8px; cursor: pointer; margin-top: 1rem; font-size: 1rem; text-transform: uppercase; letter-spacing: 1px; transition: transform 0.2s, box-shadow 0.2s; }
        .btn-confirm:hover { transform: translateY(-2px); box-shadow: 0 10px 20px rgba(239, 68, 68, 0.3); }
        ...
        .alert-warning { background: rgba(245, 158, 11, 0.2); color: #fbbf24; border: 1px solid #fbbf24; }
        
        @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
        @keyframes slideUp { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } }
        @media (max-width: 900px) { .main-grid { grid-template-columns: 1fr; } .stats-row { grid-template-columns: 1fr; } }
    </style>
</head>
<body>
...
<div class="container">
    ...
    <header class="page-header">
        <h1 class="page-title">💀 Animal Mortality Processing</h1>
        <p style="color: #64748b;">Record Deaths & Recovered Costs</p>
    </header>
    ...
    <div class="main-grid">
        ...
        <div class="glass-card">
            <h3 style="color: #ef4444; margin-bottom: 1.5rem; display: flex; align-items: center; gap: 8px;">🔍 Find Animal</h3>
            ...
            <div class="form-group"><label>4. Select Tag</label><select id="animal_id_select" class="form-select" onchange="fetchAnimalCosts(this.value)" disabled><option value="">-- Select Animal Tag --</option></select></div>
        ...
        <div class="right-column-wrapper">
            ...
            <div id="dashboard_view">
                <div class="stats-row">
                    <div class="stat-box"><div class="stat-label">Deaths Today</div><div class="stat-num" style="color: #ef4444;"><?= number_format($stats['total_deaths']) ?></div></div>
                    <div class="stat-box"><div class="stat-label">Total Cost of Deceased</div><div class="stat-num" style="color: #cbd5e1;">₱<?= number_format($stats['total_lost_investment'], 2) ?></div></div>
                    
                    <?php 
                        $impact = $stats['net_impact']; 
                        $impactColor = ($impact >= 0) ? '#34d399' : '#ef4444';
                    ?>
                    <div class="stat-box"><div class="stat-label">Net Impact (Today)</div><div class="stat-num" style="color: <?= $impactColor ?>;"><?= $impact >= 0 ? '+' : '' ?>₱<?= number_format($impact, 2) ?></div></div>
                </div>

                <div class="glass-card">
                    <h3 style="color: #94a3b8; font-size: 0.9rem; text-transform: uppercase; margin-bottom: 1rem;">Recent Mortality Records</h3>
                    <?php if(empty($recent_sales)): ?> <p style="color: #64748b; text-align: center; padding: 2rem;">No recent records found.</p> <?php else: ?>
                        <table class="recent-table">
                            <thead><tr><th>Tag</th><th>Buyer/Ref</th><th>Date</th><th style="text-align: right;">Net Impact</th></tr></thead>
                            <tbody>
                            <?php foreach($recent_sales as $sale): 
                                $profit = $sale['gross_profit'];
                                $pColor = ($profit >= 0) ? '#34d399' : '#ef4444';
                                $sign = ($profit >= 0) ? '+' : '';
                            ?>
                                <tr>
                                    <td style="color: #ef4444; font-weight: bold;"><?= $sale['TAG_NO'] ?></td>
                                    <td><?= htmlspecialchars($sale['customer_name']) ?></td>
                                    <td><?= date('M d, H:i', strtotime($sale['sale_date'])) ?></td>
                                    <td style="text-align: right; font-family: monospace; color: <?= $pColor ?>; font-weight:bold;">
                                        <?= $sign ?>₱<?= number_format($profit, 2) ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                            </tbody>
            ...
            <div id="salesFormContainer" class="glass-card">
                <form method="POST">
                    ...
                    <div class="section-header">
                        <div><div style="color: #94a3b8; font-size: 0.8rem;">RECORDING DEATH FOR</div><div class="tag-badge" id="disp_tag_header"></div></div>
                        <button type="button" class="btn-cancel" onclick="resetView()">Cancel / Close</button>
                    </div>

                    <div class="form-group">
                        <label>Date & Time of Death</label>
                        <input type="datetime-local" name="mortality_date" id="mortality_date" class="form-input" required value="<?= date('Y-m-d\TH:i') ?>">
                    </div>

                    <div class="form-group">
                        <label>Buyer / Recipient (Optional)</label>
                        ...
                        <small style="color: #64748b; font-size: 0.75rem;">If carcass was sold/transferred to a registered buyer.</small>
                    </div>

                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                        <div class="form-group"><label>Weight at Death (kg)</label><input type="number" step="0.01" name="weight_at_death" id="weight" class="form-input"></div>

                        <div class="form-group">
                            <label style="color: #86efac;">Recovered Cost (Optional)</label>
                            <input type="number" step="0.01" name="recovered_cost" id="recovered_cost" class="form-input" placeholder="0.00" value="0.00" oninput="calculateProfit()" style="border-color: #86efac;">
                            <small style="color: #64748b; font-size: 0.75rem;">E.g., Sold carcass or salvage value</small>
                        </div>
                    </div>

                    <div style="background: rgba(15,23,42,0.5); padding: 1rem; border-radius: 8px; margin: 1.5rem 0;">
                        <h4 style="color: #94a3b8; font-size: 0.8rem; text-transform: uppercase; margin-bottom: 10px;">Total Investment Breakdown</h4>
                        ...
                        <div class="cost-row" style="align-items: center;"><span>Overhead (Adjustable)</span> <input type="number" name="cost_overhead" id="c_ovr" value="0.00" step="0.01" oninput="calculateProfit()" style="width: 80px; padding: 2px; background: transparent; border: 1px solid #475569; color: #ef4444; text-align: right;"></div>
                        <div class="cost-row total-row"><span>TOTAL INVESTED</span> <span id="disp_total_cost" style="color: #ef4444;">₱0.00</span></div>
                    </div>

                    <div class="form-group">
                        <label>Reason for Loss <span style="color: #ef4444;">*</span></label>
                        <select name="loss_reason" class="form-select" required>
                    ...
                    <div class="form-group">
                        <label>Specific Details / Notes</label>
                        <textarea name="notes" class="form-textarea" placeholder="E.g. Respiratory failure, Theft incident report..."></textarea>
                    </div>

                    <div id="profitBox" class="profit-box profit-neg">
                        <div style="color: #94a3b8; font-size: 0.8rem; text-transform: uppercase;" id="profitLabel">Net Loss</div>
                        <div style="font-size: 2.5rem; font-weight: 800; margin-top: 5px;" id="profitDisplay">-₱0.00</div>
                    </div>

                    <button type="submit" name="confirm_mortality" class="btn-confirm" onclick="return confirm('Confirm mortality record? This will mark the animal as Deceased.')">Confirm Record</button>
                </form>
            </div>
        </div>
    </div>
...
<script>
    ...
    function calculateProfit() {
        ...
        const recovered = parseFloat(document.getElementById('recovered_cost').value)||0;
        const profit = recovered - currentCost; 
        
        // Dynamic Update based on Profit/Loss
        ...
        display.innerText = (profit >= 0 ? "+" : "-") + "₱" + fmt(Math.abs(profit));
        
        if(profit >= 0) {
             label.innerText = "Net Recovery (Gain)";
             pBox.className = "profit-box profit-pos";
             pBox.style.borderColor = "#10b981";
             pBox.style.background = "rgba(16, 185, 129, 0.1)";
             pBox.style.color = "#10b981";
        } else {
             label.innerText = "Net Loss";
             pBox.className = "profit-box profit-neg";
             pBox.style.borderColor = "#ef4444";
             pBox.style.background = "rgba(239, 68, 68, 0.1)";
