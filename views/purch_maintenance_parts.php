<?php
// views/purch_maintenance_parts.php
error_reporting(0);
...
// --- CONFIGURATION ---
$ITEM_TYPE_ID = 8; // Maintenance Parts
// ---------------------
...
<head>
    ...
    <title>Maintenance Parts Management | FarmPro</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        body {
            ...
            min-height: 100vh;
            padding-bottom: 60px;
            background-image: radial-gradient(ellipse 80% 50% at 50% -20%, rgba(14, 165, 233, 0.08) 0%, transparent 60%);
        }
        ...
        .page-badge {
            ...
            color: var(--accent); background: var(--accent-dim); border: 1px solid rgba(14, 165, 233, 0.2);
        ...
        .btn-primary:hover { background: #38bdf8; box-shadow: 0 0 16px var(--accent-glow); transform: translateY(-1px); }
        ...
        .form-control:focus, .form-select:focus, textarea.form-control:focus { border-color: var(--accent); box-shadow: 0 0 0 3px var(--accent-glow); background: var(--bg-hover); }
        ...
        .info-group h3 { font-size: 0.9rem; text-transform: uppercase; letter-spacing: 0.05em; color: var(--accent); margin: 1.5rem 0 1rem 0; border-bottom: 1px solid var(--border); padding-bottom: 8px; }
...
<div class="container">
    ...
    <div class="top-bar">
        ...
        <span class="page-badge"><i class="fa-solid fa-screwdriver-wrench"></i> Hardware & Tools</span>
    </div>

    <div class="page-header">
        <div class="header-info">
            <h1>Maintenance <span>Parts</span></h1>
            <p>Log and manage infrastructure upkeep, spare parts, and repair tools.</p>
        </div>
    ...
    <div class="search-container">
        ...
        <input type="text" class="search-input" id="searchInput" placeholder="Quick search by part name, supplier, or reference no..." onkeyup="filterTable()">
    </div>
                        ...
                        <tr>
                            <td colspan="12">
                                <div class="empty-state">
                                    ...
                                    No maintenance parts purchases recorded yet in this location.
                                </div>
                            </td>
                        </tr>
                    <?php else: ?>
...
<div id="modal" class="modal">
    <div class="modal-content">
        <div class="modal-header">
            <h2 id="modal-title">Add Maintenance Parts Purchase</h2>
        </div>
            ...
            <form id="item-form" method="POST">
                ...
                <div class="info-group" style="margin-top: 0;">
                    ...
                    <div class="form-group autocomplete-wrapper">
                        <label class="form-label" for="item-name">Part/Item Name <span>*</span></label>
                        <input type="text" id="item-name" name="item_name" class="form-control" placeholder="e.g., Replacement Belt, Motor" required maxlength="300" autocomplete="off">
                    ...
                    <div class="form-row">
                        <div class="form-group autocomplete-wrapper">
                            ...
                            <input type="text" id="supplier" name="supplier" class="form-control" placeholder="e.g., Industrial Tools Corp" autocomplete="off">
...
<div id="confirm-modal" class="modal">
    <div class="modal-content" style="max-width: 450px;">
        <div class="modal-body confirm-content">
            <span class="confirm-icon" style="color:var(--accent);">⚙️</span>
            <h2 style="color: #fff; margin-bottom: 10px;">Verify Acquisition?</h2>
            <p style="color: var(--text-secondary); margin-bottom: 5px;">You are confirming the intake of <strong><span id="confirm-item-qty"></span> <span id="confirm-item-name" style="color:var(--accent);"></span></strong>.</p>
...
<div id="confirm-all-modal" class="modal">
    <div class="modal-content" style="max-width: 450px;">
        <div class="modal-body confirm-content">
            ...
            <p style="color: var(--text-secondary);">This function will verify and lock <strong>ALL</strong> currently pending maintenance parts purchases in this location.</p>
...
<form id="deleteItemForm" method="POST" action="../process/deleteMaintenanceParts.php" style="display: none;">
...
<script>
    ...
    function submitConfirmation() {
        ...
        fetch('../purchase_confirmations/confirmMaintenanceAndParts.php', { method: 'POST', body: formData })
            .then(r => r.json())
            .then(data => {
    ...
    function submitConfirmAll() {
        ...
        fetch('../purchase_confirmations/confirmAllMaintenanceAndParts.php', { method: 'POST' })
            .then(r => r.json())
            .then(data => {
                if (data.success) { showToast(data.message, 'success'); setTimeout(() => window.location.reload(), 1000); } 
    ...
    function fetchAutocomplete(searchTerm) {
        fetch(`../process/searchMaintenanceAndParts.php?term=${encodeURIComponent(searchTerm)}`)
    ...
    function openAddModal() {
        document.getElementById('modal-title').textContent = 'Add Maintenance Parts Purchase';
        document.getElementById('btn-save').innerHTML = '<i class="fa-solid fa-floppy-disk me-2"></i> Save Purchase';
        document.getElementById('item-form').reset();
        document.getElementById('item-id').value = '';
        document.getElementById('item-category').value = '0'; // Default Non-Consumable
    ...
    function displayItemDetails(data) {
        ...
        const html = `
            <div class="info-group">
                ...
                <p style="margin-top:15px; color:var(--text-primary);"><strong>Part/Item Name:</strong> <br>${data.item_name}</p>
            ...
            <div class="info-group" style="margin-top:20px;">
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
                    ...
                    <p style="color:var(--text-secondary); margin:0;"><strong>Net Weight:</strong> <br><span class="val-mono">${data.net_weight || '0'}</span></p>
                </div>
    ...
    function populateEditForm(data) {
        document.getElementById('modal-title').textContent = 'Edit Maintenance Parts Purchase';
    ...
    function saveItem() {
        ...
        const url = isEdit ? '../process/editMaintenanceAndParts.php' : '../process/addMaintenanceAndParts.php';
    ...
    function showToast(msg, type = 'success') {
        ...
        t.className = 'toast';
        t.style.borderLeft = `4px solid ${type === 'error' ? 'var(--red)' : 'var(--accent)'}`;
