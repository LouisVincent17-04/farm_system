...
try {
    ...
    if ($date_from && $date_to) { 
        $where .= " AND DATE(ft.TRANSACTION_DATE) BETWEEN :df AND :dt";
        $params[':df'] = $date_from; $params[':dt'] = $date_to;
    }
    ...
    $sql = "SELECT 
                ...
                $select_tag,
                COALESCE(f.FEED_NAME, 'Deleted Feed Batch') AS FEED_NAME,
    ...
    foreach ($results as $row) {
        ...
        if(!isset($grouped_data[$header])) {
            $grouped_data[$header] = [ 'items' => [], 'sub_kg' => 0, 'sub_cost' => 0 ];
        }

        $feed_key = strtolower(trim($row['LOCATION_NAME'])) . '_' . strtolower(trim($row['FEED_NAME']));
        $row['PURCHASED_QTY'] = $purchases_by_feed[$feed_key]['qty'] ?? 0;
        $row['PURCHASED_COST'] = $purchases_by_feed[$feed_key]['cost'] ?? 0;

        if(isset($purchases_by_feed[$feed_key])) {
            $purchases_by_feed[$feed_key]['processed'] = true;
        }

        $grouped_data[$header]['items'][] = $row;
        $grouped_data[$header]['sub_kg'] += $row['TOTAL_KGS'];
        $grouped_data[$header]['sub_cost'] += $row['TOTAL_COST'];

        $grand_total_kg += $row['TOTAL_KGS'];
    ...
    if ($f_loc) {
        $adj_where .= " AND f.LOCATION_ID = :aloc";
    ...
    $adj_sql = "SELECT 
                ...
                LEFT JOIN feeds f ON ia.REF_ID = f.FEED_ID
    ...
    $stock_sql = "SELECT SUM(TOTAL_WEIGHT_KG) as current_stock FROM feeds $stock_where";
...
<head>
    ...
    <title>Feeds Usage & Accountancy Report</title>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .title { font-size: clamp(1.8rem, 4vw, 2.5rem); font-weight: 800; background: linear-gradient(135deg, #3b82f6, #06b6d4); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom: 0.5rem; }
        ...
        /* Changed to exactly 3 columns on desktop to prevent compression */
        ...
        .group-header { background: rgba(15, 23, 42, 0.8); padding: 1rem 1.5rem; font-weight: 700; color: #93c5fd; border-bottom: 1px solid #334155; font-size: 1.1rem; display: flex; align-items: center; gap: 10px; }
        ...
        .row-total { background: rgba(59, 130, 246, 0.1); font-weight: bold; border-top: 1px solid #334155 !important; }
...
<div class="container">
    ...
    <div class="header">
        <h1 class="title">Feeds Usage & Ledger Report</h1>
        <p class="subtitle">Accountancy overview of feed purchases, consumption, adjustments, and live balances.</p>
    ...
    <div class="acc-summary-grid">
        <div class="acc-card c-inwards">
            ...
            <div class="acc-val"><?= number_format($purchased_qty, 2) ?> kg</div>
            <div class="acc-sub">₱ <?= number_format($purchased_cost, 2) ?></div>
        </div>
        
        <div class="acc-card c-outwards">
            <h4>📤 Total Consumed (OUT)</h4>
            <div class="acc-val"><?= number_format($grand_total_kg, 2) ?> kg</div>
        ...
        <div class="acc-card c-variance">
            ...
            <?php 
                $var_qty = $purchased_qty - $grand_total_kg;
                $var_cost = $purchased_cost - $grand_total_cost;
            ?>
            <div class="acc-val"><?= number_format($var_qty, 2) ?> kg</div>
        ...
        <div class="acc-card c-adjust">
            ...
            ?>
            <div class="acc-val" style="color:<?= $adj_color ?>;"><?= $adj_sign . number_format($net_adjustments, 2) ?> kg</div>
        ...
        <div class="acc-card c-adj-net">
            ...
            ?>
            <div class="acc-val"><?= number_format($expected_net, 2) ?> kg</div>
            <div class="acc-sub">IN - OUT + ADJ</div>
        </div>

        <div class="acc-card c-stock">
            <h4>📦 Live Warehouse Stock</h4>
            <div class="acc-val"><?= number_format($current_stock, 2) ?> kg</div>
            ...
            <div class="filter-grid">
                ...
                <div class="form-group" style="grid-column: 1 / -1;">
                    <div class="btn-group">
                        ...
                        <a href="feeds_usage_report.php" class="btn btn-outline">Reset</a>
                    </div>
                </div>
            </div>
    ...
    <?php if(empty($grouped_data) && $purchased_qty == 0 && empty($adjustments_data)): ?>
        <div class="empty-state">
            <h2 style="margin-bottom:10px;">No Feed Activity Found</h2>
    ...
    <?php else: ?>
        ...
        <?php foreach ($grouped_data as $header => $group): ?>
            <div class="group-container">
                <div class="group-header">
                    <i class="fa-solid fa-layer-group"></i> <?= htmlspecialchars($header) ?>
                </div>
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php foreach ($group['items'] as $item): ?>
                                <tr>
                                    <td data-label="Feed Name" style="font-weight:600; color:#fff;"><?= htmlspecialchars($item['FEED_NAME']) ?></td>
                                    
                                    <td data-label="Purchased (IN)" class="t-right" style="color:#10b981;">
                                        <?= number_format($item['PURCHASED_QTY'], 2) ?> kg
                                    </td>
                                    
                                    <td data-label="Used KGs (OUT)" class="t-right" style="color:#60a5fa; font-weight:bold;"><?= number_format($item['TOTAL_KGS'], 2) ?> kg</td>
                            ...
                            <tr class="row-total">
                                ...
                                <td data-label="Used KGs (OUT)" class="t-right"><?= number_format($group['sub_kg'], 2) ?> kg</td>
        ...
        <?php if(!empty($adjustments_data)): ?>
            <div class="group-container" style="border-color: #a855f7;">
                <div class="group-header adjustments">
                    <i class="fa-solid fa-wrench"></i> INVENTORY ADJUSTMENTS (FEEDS)
                </div>
                <div class="table-wrap">
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th>Feed Name</th>
                                <th>Type</th>
                                <th class="t-right">Quantity Adjusted</th>
                        ...
                        <tbody>
                            <?php foreach ($adjustments_data as $adj): ?>
                                <tr>
                                    ...
                                    <td data-label="Feed Name" style="font-weight:600; color:#fff;"><?= htmlspecialchars($adj['ITEM_NAME']) ?></td>
                                    <td data-label="Type">
                                    ...
                                    <td data-label="Quantity" class="t-right" style="font-weight:bold; color: <?= strtolower($adj['ADJUSTMENT_TYPE']) == 'add' ? '#34d399' : '#f87171' ?>;">
                                        <?= strtolower($adj['ADJUSTMENT_TYPE']) == 'add' ? '+' : '-' ?><?= number_format($adj['QUANTITY'], 2) ?> kg
                                    </td>
        ...
        <?php 
        $has_unused = false;
        foreach($purchases_by_feed as $fName => $pData) {
        ...
        ?>
            <div class="group-container" style="border-color: #10b981;">
                <div class="group-header warehouse">
                    <i class="fa-solid fa-warehouse"></i> WAREHOUSE STOCK (Purchased but 0 Usage in Pens)
                </div>
                <div class="table-wrap">
                    <table>
                        <thead>
                        ...
                        <tbody>
                            <?php foreach ($purchases_by_feed as $fName => $pData): ?>
                                ...
                                <tr>
                                    ...
                                    <td data-label="Feed Name" style="font-weight:600; color:#fff;"><?= htmlspecialchars($pData['original_name']) ?></td>
                                    <td data-label="Purchased (IN)" class="t-right" style="color:#10b981; font-weight:bold;">
                                        <?= number_format($pData['qty'], 2) ?> kg
                                    </td>
                                    <td data-label="Used KGs (OUT)" class="t-right" style="color:#64748b;">0.00 kg</td>
        ...
        <div class="group-container" style="border-color: #3b82f6;">
            <div class="group-header" style="background: rgba(59, 130, 246, 0.2); color: #fff;">
                <i class="fa-solid fa-chart-pie"></i> GRAND TOTAL (Filtered Outwards)
            </div>
            <div class="table-wrap">
                <table>
                    <tbody>
                        <tr class="row-total" style="font-size: 1.1rem;">
                            <td data-label="Summary" style="color: #3b82f6;">OVERALL USAGE</td>
                            <td data-label="Used KGs (OUT)" class="t-right" style="color: #60a5fa;"><?= number_format($grand_total_kg, 2) ?> kg</td>
...
<script>
    ...
    const purchasesData = <?php echo json_encode($purchases_by_feed); ?>;
    const adjustmentsData = <?php echo json_encode($adjustments_data); ?>;
    
    const grandTotals = { kg: "<?= number_format($grand_total_kg, 2) ?>", cost: "<?= number_format($grand_total_cost, 2) ?>" };
    const accSummary = {
    ...
    function exportCSV() {
        ...
        link.setAttribute("download", `Feeds_Ledger_Report_${filenameDate}.csv`);
