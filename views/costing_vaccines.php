    ...
    try {
            ...
            if ($stmt->rowCount() > 0) {
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    $html .= "<tr>
                                ...
                                <td data-label=\"Date\" class=\"td-date\">{$date}</td>
                                <td data-label=\"Vaccine Name\" style=\"font-weight:700; color:#fff;\">{$row['VACCINE_NAME']}</td>
                                <td data-label=\"Dosage\" class=\"td-mono\">{$row['QUANTITY']} <span style=\"color:var(--text-muted);\">ml</span></td>
                                <td data-label=\"Vaccine Cost\" class=\"td-mono\" style=\"color:var(--text-secondary);\">₱ " . number_format($vac_cost, 2) . "</td>
                                <td data-label=\"Service Fee\" class=\"td-mono\" style=\"color:var(--text-secondary);\">₱ " . number_format($svc_cost, 2) . "</td>
                                <td data-label=\"Total Cost\" class=\"td-cost\">₱ " . number_format($row_total, 2) . "</td>
                                <td data-label=\"Remarks\" style=\"color:var(--text-secondary); font-size:0.85rem;\">" . htmlspecialchars($row['REMARKS'] ?? '-') . "</td>
                              </tr>";
                }
            } else {
                $html = "<tr><td colspan='8' class='empty-state'><i class='fa-solid fa-ghost' style='font-size:2rem; display:block; margin-bottom:1rem; opacity:0.5;'></i> No vaccination history found for this pen.</td></tr>";
            }
        ...
        if ($action === 'get_history' && isset($_GET['animal_id'])) {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $html = "<tr><td colspan='7' class='empty-state'><i class='fa-solid fa-ghost' style='font-size:2rem; display:block; margin-bottom:1rem; opacity:0.5;'></i> No vaccination history found for this animal.</td></tr>";
            }
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .data-table td { padding: 14px 16px; border-bottom: 1px solid rgba(255,255,255,0.03); color: var(--text-primary); vertical-align: middle; white-space: nowrap;}
        .data-table tr:hover { background: rgba(255,255,255,0.01); }

        .tag-badge { background: var(--pink-dim); color: var(--pink); padding: 4px 10px; border-radius: 6px; font-size: 0.85rem; font-weight: 700; font-family: var(--font-mono); border: 1px solid rgba(236,72,153,0.3); display: inline-flex; align-items: center; gap: 4px;}
        ...
        .td-cost { color: var(--pink); font-weight: 700; font-family: var(--font-mono); font-size: 1.05rem;}
        ...
        .total-box .val { color: var(--pink); font-size: 2rem; font-weight: 800; font-family: var(--font-mono); line-height: 1;}
        ...
        .scroll-hint-icon {
            ...
            color: var(--pink); filter: drop-shadow(0 4px 15px rgba(236,72,153,0.5));
        }
        .scroll-hint-text {
...
<div class="container">
    ...
    <div class="top-bar">
        ...
        <span class="page-badge"><i class="fa-solid fa-syringe"></i> Immunization Analytics</span>
    </div>

    <header class="page-header">
        <div class="header-info">
            <h1>Vaccination Cost <span>History</span></h1>
            <p>Analyze historical immunization usage and associated batch costs.</p>
        </div>
    </header>
        ...
        <div class="workspace-panel">
            <div id="resultsArea" class="table-section">
                ...
                <div class="section-header">
                    <div>
                        <div class="section-title"><i class="fa-solid fa-list-ul"></i> Vaccination Transactions</div>
                ...
                <div class="total-box">
                    <span class="lbl">Total Vaccination Cost</span>
