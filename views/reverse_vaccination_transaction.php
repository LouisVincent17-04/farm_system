...
try {
    ...
    // 1. Fetch the VERY latest vaccination transaction
    $sql = "SELECT 
                v.VACCINATION_ID,
                v.VACCINATION_DATE,
                v.QUANTITY,
...
<head>
    <meta charset="UTF-8">
    <title>Admin - Reverse Vaccination</title>
    <style>
        :root { --dark: #0f172a; --dark-light: #1e293b; --red: #ef4444; --green: #10b981; }
        ...
        .alert-warning { background: rgba(16, 185, 129, 0.1); color: #6ee7b7; border: 1px solid #10b981; }
...
<div class="container">
    <div class="card">
        <div class="header">
            ...
            <p>Undo the most recent vaccination record.</p>
        </div>

        <?php if ($last_trans): 
            $total_cost = $last_trans['VACCINE_COST'] + $last_trans['VACCINATION_COST'];    
        ?>
            <div class="alert alert-warning">
                ⚠️ <strong>Confirmation:</strong> This will delete the record below and restore vaccine stock.
            </div>
            ...
            <div class="trans-info">
                ...
                <div class="info-row">
                    ...
                    <span class="value" style="color:#34d399;">#<?= htmlspecialchars($last_trans['VACCINATION_ID']) ?></span>
                </div>
                <div class="info-row">
                    <span class="label">Date Administered</span>
                    <span class="value"><?= date('M d, Y h:i A', strtotime($last_trans['VACCINATION_DATE'])) ?></span>
                </div>
                <div class="info-row">
                ...
                <div class="info-row">
                    ...
                    <span class="value"><?= htmlspecialchars($last_trans['VAC_NAME']) ?></span>
                </div>
                <div class="info-row">
                    <span class="label">Quantity Used</span>
                    <span class="value" style="color:#fbbf24;">
                        <?= number_format($last_trans['QUANTITY'], 2) ?> <?= $last_trans['UNIT_ABBR'] ?> 
                    </span>
                </div>
                <div class="info-row">
                    <span class="label">Total Cost</span>
                    <span class="value">₱<?= number_format($total_cost, 2) ?></span>
                </div>
                <div class="info-row">
                    <span class="label">Veterinarian</span>
                    <span class="value" style="font-size:0.85rem;"><?= htmlspecialchars($last_trans['VET_NAME']) ?></span>
                </div>
            </div>
...
<script>
    function confirmReversal() {
        if(!confirm("🔴 DANGER: Are you sure you want to delete this vaccination record?\n\nInventory will be restored.")) {
            return;
        }
