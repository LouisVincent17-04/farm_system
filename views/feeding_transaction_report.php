<?php
// reports/feed_transaction_report.php
error_reporting(0);
...
try {
    ...
    // --- 2. BUILD SQL QUERY ---
    // Joins: 
    // - FEEDS (to get Feed Name from FEED_ID)
    ...
    $sql = "SELECT 
            ...
            ft.REMARKS,
            ft.BATCH_ID,
            DATE_FORMAT(ft.TRANSACTION_DATE, '%Y-%m-%d %H:%i') as FEED_DATE,
            f.FEED_NAME,
            ar.TAG_NO
    ...
    if ($date_from && $date_to) {
        $sql .= " AND ft.TRANSACTION_DATE BETWEEN :date_from AND :date_to";
        $params[':date_from'] = $date_from . ' 00:00:00';
        $params[':date_to']   = $date_to . ' 23:59:59';
    }

    // Filter: Search (Tag, Feed Name, Remarks, Batch ID)
    if ($search_term) {
        $sql .= " AND (ar.TAG_NO LIKE :search OR f.FEED_NAME LIKE :search OR ft.REMARKS LIKE :search OR ft.BATCH_ID LIKE :search)";
...
<head>
    ...
    <title>Feed Consumption History</title>
...
<div class="container">
    <div class="header">
        <h1 class="title">Feed Consumption History</h1>
        <p class="subtitle">Log of feed usage transactions, costs, and batch tracking.</p>
    </div>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card">
            <div class="stat-lbl">Total Consumed (KG)</div>
            <div class="stat-val text-amber"><?= number_format($total_kg_used, 2) ?></div>
        </div>
        <div class="stat-card">
            <div class="stat-lbl">Total Cost</div>
            <div class="stat-val text-orange">₱<?= number_format($total_cost, 2) ?></div>
        </div>
    </div>
    ...
    <div class="filter-box">
        <form method="GET">
            <div class="filter-grid">
                ...
                <div class="form-group">
                    <label>Search (Tag / Feed / Batch)</label>
                    <input type="text" name="search" class="form-input" placeholder="e.g. Pig 01, Starter, Batch-123" value="<?= htmlspecialchars($search_term) ?>">
                </div>
                
                <div class="btn-group">
                    <button type="submit" class="btn btn-primary">Apply Filters</button>
                    <a href="feed_transaction_report.php" class="btn btn-outline">Reset</a>
                </div>
            </div>
        ...
        <table>
            ...
            <tbody>
                <?php if(empty($transactions)): ?>
                    <tr><td colspan="7" style="text-align:center; padding:3rem; color:#64748b;">No feed transactions found.</td></tr>
                <?php else: ?>
                    <?php foreach($transactions as $t): ?>
                    <tr>
                        <td style="color:#fbbf24; font-weight:bold;"><?= $t['FEED_DATE'] ?></td>
                        <td>
                            <div style="font-weight:bold; color:#fff;"><?= htmlspecialchars($t['TAG_NO'] ?? 'Unknown Tag') ?></div>
                        </td>
                        <td><?= htmlspecialchars($t['FEED_NAME'] ?? 'Unknown Feed') ?></td>
                        <td style="font-family:monospace; color:#94a3b8; font-size:0.8rem;"><?= htmlspecialchars($t['BATCH_ID'] ?? '-') ?></td>
                        <td style="text-align:right; font-weight:bold;">
                            <?= number_format($t['QUANTITY_KG'], 2) ?>
                        </td>
                        <td style="text-align:right; color:#fb923c;">₱<?= number_format($t['TRANSACTION_COST'], 2) ?></td>
...
<script>
    ...
    const records = <?php echo json_encode($transactions); ?>;
    const stats = {
    ...
    function exportPDF() {
        ...
        doc.text("Feed Consumption Report", 14, 15);
        
        doc.setFontSize(10);
        doc.setTextColor(100);
        const dateStr = new Date().toLocaleString();
        doc.text(`Generated: ${dateStr}`, 14, 22);
        doc.text(`Total Records: ${stats.totalCount} | Total Cost: PHP ${stats.totalCost}`, 14, 28);
        ...
        const rows = records.map(r => [
            ...
            parseFloat(r.TRANSACTION_COST).toFixed(2),
            r.REMARKS || '-'
        ]);

        doc.autoTable({
            head: [['Date', 'Tag', 'Feed', 'Batch ID', 'Qty (KG)', 'Cost', 'Remarks']],
            body: rows,
            startY: 35,
            styles: { fontSize: 8 },
            headStyles: { fillColor: [180, 83, 9] } // Dark Amber Header
        });

        doc.save('Feed_Transaction_Report.pdf');
    }
    ...
    function exportExcel() {
        const excelData = records.map(r => ({
            ...
            'Feed Name': r.FEED_NAME || 'Unknown',
            'Batch ID': r.BATCH_ID || '-',
            'Quantity (KG)': parseFloat(r.QUANTITY_KG),
            'Cost': parseFloat(r.TRANSACTION_COST),
            'Remarks': r.REMARKS
        }));

        const ws = XLSX.utils.json_to_sheet(excelData);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Feed Transactions");
        XLSX.writeFile(wb, "Feed_Transaction_Report_" + new Date().toISOString().slice(0,10) + ".xlsx");
    }

    // --- CSV Export ---
    function exportCSV() {
        let csvContent = "data:text/csv;charset=utf-8,";
        csvContent += "Date,Tag,Feed,Batch,Qty KG,Cost,Remarks\n";
        
        records.forEach(r => {
            const row = [
                r.FEED_DATE, r.TAG_NO, r.FEED_NAME, r.BATCH_ID,
                r.QUANTITY_KG, r.TRANSACTION_COST, r.REMARKS
        ...
        link.setAttribute("download", "Feed_Transaction_Report_" + new Date().toISOString().slice(0,10) + ".csv");
