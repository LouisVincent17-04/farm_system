...
try {
    ...
    // --- 2. BUILD SQL QUERY ---
    // Fetch items where ITEM_TYPE_ID = 5 (Sanitation & Waste Manage)
    $sql = "SELECT 
            i.ITEM_ID,
            i.ITEM_NAME,
            i.ITEM_DESCRIPTION,
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body {
            ...
            background-image:
                ...
                radial-gradient(ellipse 40% 30% at 85% 10%, rgba(20,184,166,0.04) 0%, transparent 50%);
        }
        ...
        .stat-card.emerald::before { background: linear-gradient(135deg, rgba(16,185,129,0.04), transparent); }
        .stat-card.teal::before    { background: linear-gradient(135deg, rgba(20,184,166,0.04), transparent); }
        .stat-card.green::before   { background: linear-gradient(135deg, rgba(34,197,94,0.04), transparent); }

        .stat-card.emerald { border-color: rgba(16,185,129,0.15); }
        .stat-card.teal    { border-color: rgba(20,184,166,0.15); }
        ...
        .filter-badge {
            ...
            background: var(--emerald); color: #fff; border-radius: 99px; padding: 0 6px;
        }
        .filter-toggle-btn {
        ...
        .form-control:focus {
            border-color: var(--border-active); box-shadow: 0 0 0 3px var(--emerald-glow);
        ...
        .btn-primary:hover { background: #059669; box-shadow: 0 0 16px var(--emerald-glow); color: #fff; }
        ...
        /* ─── VALUE CELLS ─── */
        .val-money { font-family: var(--font-mono); color: var(--emerald); font-size: 0.8rem; font-weight: 600; }
        .val-cost  { font-family: var(--font-mono); color: var(--text-secondary); font-size: 0.8rem; }
        .val-count { font-family: var(--font-mono); color: var(--teal); font-size: 0.85rem; font-weight: 600; }
...
<div class="container">
    ...
    <div class="top-bar">
        ...
        <span class="page-badge"><i class="fa-solid fa-trash-can"></i> Sanitation & Waste</span>
    </div>

    <div class="page-header">
        <h1 class="page-title">Sanitation <span>& Waste</span> Report</h1>
        <p class="page-subtitle">Log of sanitation supplies and waste management acquisitions.</p>
    </div>

    <div class="stats-grid">
        <div class="stat-card emerald">
            <div class="stat-icon emerald"><i class="fa-solid fa-boxes-packing"></i></div>
            <div class="stat-val emerald"><?= number_format($unique_items_count) ?></div>
            <div class="stat-lbl">Unique Items</div>
        </div>
        <div class="stat-card teal">
            <div class="stat-icon teal"><i class="fa-solid fa-boxes-stacked"></i></div>
            <div class="stat-val teal"><?= number_format($total_units) ?></div>
    ...
    <div class="filter-panel">
            ...
            <form method="GET" id="filterForm">
                <div class="filter-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="search" class="form-control" placeholder="e.g. Disinfectant, Bags" value="<?= htmlspecialchars($search_term) ?>">
                    </div>
        ...
        <div class="filter-footer">
            <div class="filter-footer-left">
                <a href="sanitation_report.php" class="btn btn-ghost btn-sm">
                        ...
                        <tr>
                            <td colspan="7">
                                <div class="empty-state">
                                    ...
                                    <p>No sanitation records found matching your filters.</p>
                                </div>
                            </td>
                        </tr>
                    <?php else: ?>
...
<script>
    ...
    function exportPDF() {
        ...
        doc.text("Sanitation & Waste Purchase Report", 14, 14);
        ...
        doc.save('Sanitation_Report.pdf');
    }
    ...
    function exportExcel() {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Sanitation");
        XLSX.writeFile(wb, "Sanitation_Report_" + new Date().toISOString().slice(0,10) + ".xlsx");
    }
    ...
    function exportCSV() {
        ...
        link.setAttribute("download", "Sanitation_Report_" + new Date().toISOString().slice(0,10) + ".csv");
