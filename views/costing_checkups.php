    ...
    try {
            ...
            if ($stmt->rowCount() > 0) {
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    $html .= "<tr>
                                ...
                                <td data-label=\"Veterinarian\" style=\"font-weight:700; color:#fff;\">{$row['VET_NAME']}</td>
                                <td data-label=\"Professional Fee\" class=\"td-cost\">₱ {$cost}</td>
                                <td data-label=\"Remarks\" style=\"color:var(--text-secondary); font-size:0.85rem;\">" . htmlspecialchars($row['REMARKS'] ?? '-') . "</td>
                              </tr>";
                }
            } else {
                $html = "<tr><td colspan='5' class='empty-state'><i class='fa-solid fa-ghost' style='font-size:2rem; display:block; margin-bottom:1rem; opacity:0.5;'></i> No check-up history found for this pen.</td></tr>";
            }
        ...
        if ($action === 'get_history' && isset($_GET['animal_id'])) {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $html = "<tr><td colspan='4' class='empty-state'><i class='fa-solid fa-ghost' style='font-size:2rem; display:block; margin-bottom:1rem; opacity:0.5;'></i> No check-up history found for this animal.</td></tr>";
            }
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .header-info h1 span { background: linear-gradient(135deg, var(--purple), #6d28d9); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        ...
        .form-select:focus { border-color: var(--purple); box-shadow: 0 0 0 3px var(--purple-glow); background: var(--bg-hover); }
        ...
        .tag-badge { background: var(--purple-dim); color: var(--purple); padding: 4px 10px; border-radius: 6px; font-size: 0.85rem; font-weight: 700; font-family: var(--font-mono); border: 1px solid rgba(168,85,247,0.3); display: inline-flex; align-items: center; gap: 4px;}
        .td-date { font-size: 0.9rem; color: var(--text-secondary); font-family: var(--font-mono);}
        .td-cost { color: var(--purple); font-weight: 700; font-family: var(--font-mono); font-size: 1.05rem;}
        ...
        .total-box .val { color: var(--purple); font-size: 2rem; font-weight: 800; font-family: var(--font-mono); line-height: 1;}
        ...
        .scroll-hint-icon {
            ...
            color: var(--purple); filter: drop-shadow(0 4px 15px rgba(168,85,247,0.5));
        }
        .scroll-hint-text {
...
<div class="container">
    ...
    <div class="top-bar">
        ...
        <span class="page-badge"><i class="fa-solid fa-stethoscope"></i> Clinical Analytics</span>
    </div>

    <header class="page-header">
        <div class="header-info">
            <h1>Check-up Cost <span>History</span></h1>
            <p>Analyze historical veterinary visits, professional fees, and diagnostics.</p>
        </div>
    </header>
        ...
        <div class="workspace-panel">
            <div id="resultsArea" class="table-section">
                ...
                <div class="section-header">
                    <div>
                        <div class="section-title"><i class="fa-solid fa-list-ul"></i> Checkup Transactions</div>
                ...
                <div class="total-box">
                    <span class="lbl">Total Medical Cost</span>
