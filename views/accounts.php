...
<head>
    ...
    <style>
        ...
        /* Filter Tabs */
        .filter-tabs { display: flex; gap: 1rem; margin-bottom: 1.5rem; border-bottom: 1px solid #334155; padding-bottom: 0.5rem; overflow-x: auto; }
        ...
        .search-input { width: 100%; padding: 1rem 1rem 1rem 3rem; background: rgba(30, 41, 59, 0.5); border: 1px solid #475569; border-radius: 0.5rem; color: white; }
        ...
        .action-btn { padding: 8px; border-radius: 6px; border: none; cursor: pointer; background: transparent; transition: all 0.2s; display: inline-flex; align-items: center; justify-content: center; text-decoration: none; }
        
        /* Edit (Blue) */
        .action-btn.edit { color: #60a5fa; } 
        .action-btn.edit:hover { background: rgba(59, 130, 246, 0.2); }
        
        /* Permissions (Amber/Orange) - NEW */
        .action-btn.permissions { color: #f59e0b; } 
        .action-btn.permissions:hover { background: rgba(245, 158, 11, 0.2); }
        ...
        /* Reactivate (Green) */
        .action-btn.reactivate { color: #10b981; } 
        .action-btn.reactivate:hover { background: rgba(16, 185, 129, 0.2); }
        ...
        .modal-content { background: #1e293b; border-radius: 0.75rem; width: 100%; max-width: 28rem; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); border: 1px solid #475569; display: flex; flex-direction: column; }
        ...
        .form-group input { padding: 0.75rem; background: #374151; border: 1px solid #4b5563; border-radius: 0.5rem; color: white; width: 100%; box-sizing: border-box; }
        ...
        @media (max-width: 768px) {
            ...
            .table td { padding: 0.5rem 0; border-bottom: 1px solid rgba(255,255,255,0.05); display: flex; justify-content: space-between; align-items: center; text-align: right; }
            .table td::before { content: attr(data-label); float: left; font-weight: 600; color: #94a3b8; font-size: 0.85rem; text-transform: uppercase; margin-right: 1rem; }
            .table td[data-label="User"] { display: block; text-align: left; border-bottom: 1px solid #64748b; padding-bottom: 1rem; margin-bottom: 0.5rem; }
            .table td[data-label="User"]::before { display: none; }
            .table td:last-child { border-bottom: none; justify-content: flex-end; margin-top: 0.5rem; padding-top: 0.5rem; }
...
<body>
                            ...
                            <td data-label="Actions">
                                <div class="actions">
                                    <?php if ($isActive): ?>
                                        ...
                                        <a href="manage_access.php?user_id=<?php echo $user['USER_ID']; ?>" class="action-btn permissions" title="Manage Permissions & Role">
    ...
    <script>
        ...
        function saveAccount() {
            ...
            .then(r => r.json())
            .then(data => {
                if(data.success) { showSuccessAndReload(data.message || 'Account details updated successfully!'); }
                else { alert('Error: ' + data.message); saveBtn.disabled = false; saveBtn.textContent = 'Save Changes'; }
            })
            .catch(error => { console.error('Fetch error:', error); alert('A network error occurred.'); saveBtn.disabled = false; saveBtn.textContent = 'Save Changes'; });
        }
