...
try {
    ...
    // We group by CHECKUP_DATE to identify batches
    $sql = "SELECT 
                c.CHECKUP_DATE,
                MAX(c.VET_NAME) as VET_NAME,
                COUNT(c.CHECK_UP_ID) as record_count,
                SUM(c.COST) as total_cost
            FROM check_ups c
            GROUP BY c.CHECKUP_DATE
...
<head>
    ...
    <title>Reverse Check-up | FarmPro</title>
...
<div class="container">
    ...
    <div class="page-header">
        <h1 class="page-title"><i class="fa-solid fa-clock-rotate-left" style="color:var(--red);"></i> Admin <span>Undo</span> Checkup</h1>
        <p class="page-desc">Select a historical check-up batch to permanently delete its records and reverse associated costs.</p>
    </div>
        ...
        <div class="main-grid">
            ...
            <div class="control-panel">
                ...
                <div class="form-group">
                    ...
                    <select id="batchSelect" class="form-select" onchange="handleBatchSelection()">
                        ...
                        <?php foreach($batches as $b): ?>
                            <option value="<?= htmlspecialchars($b['CHECKUP_DATE']) ?>">
                                <?= date('M d, Y - h:i A', strtotime($b['CHECKUP_DATE'])) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="alert-box">
                    ...
                    <div>
                        <strong>Warning:</strong> This action is permanent. Reversing a batch will completely delete its transaction records and remove their service fees from the financial ledger.
                    </div>
                </div>
            </div>
            ...
            <div id="detailsPanel" class="details-panel">
                ...
                <div class="info-grid">
                    ...
                    <div class="info-row">
                        <span class="info-lbl">Veterinarian</span>
                        <span class="info-val highlight" id="lbl_person" style="font-family: var(--font); font-size: 0.95rem;">—</span>
                    </div>
                    <div class="info-row">
                    ...
                    <div class="info-row">
                        <span class="info-lbl">Total Professional Fee</span>
...
<script>
    ...
    function handleBatchSelection() {
        ...
        const batch = batchData.find(b => String(b.CHECKUP_DATE) === String(selectedDate));

        if (batch) {
            // Format Date
            const d = new Date(batch.CHECKUP_DATE);
            ...
            document.getElementById('lbl_person').textContent = batch.VET_NAME || 'Unknown';
    ...
    function confirmReversal() {
        ...
        if(!confirm(`DANGER: Are you absolutely sure you want to delete this check-up batch?\n\nThis will permanently remove the transactions and operational costs from the ledger.`)) {
            return;
        }
