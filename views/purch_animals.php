<?php
// views/purch_animal_purchases.php
error_reporting(0);
...
try {
    ...
    // 2. STRICT UNIT LOGIC
    // MySQL: Use LIMIT 1 instead of FETCH FIRST
...
<head>
    ...
    <link rel="stylesheet" href="../css/purch_farm_equipment_tools.css">
    <style>
        ...
        /* --- CONFIRM ALL BUTTON & HEADER ACTIONS --- */
        .header-actions {
            display: flex;
            gap: 10px;
            align-items: center;
...
<body>
    <div class="container">
        ...
        <div class="table-container">
            <table class="table">
                ...
                <tbody id="item-table">
                    ...
                    <tr data-item-id="<?php echo $item['ITEM_ID']; ?>"
                        ...
                        <td>
                            <div class="item-id">ANM-<?php echo str_pad($item['ITEM_ID'], 4, '0', STR_PAD_LEFT); ?></div>
                        </td>
                        <td>
            ...
            <div id="empty-state" class="empty-state">
                <h3>No animal purchases found</h3>
                ...
                <form id="item-form" method="POST">
                    ...
                    <div class="info-group">
                        ...
                        <div class="form-group autocomplete-wrapper">
                            <label for="item-name">Animal <span>*</span></label>
                            <input type="text" id="item-name" name="item_name" placeholder="e.g., Hog, Chicken" required maxlength="300" autocomplete="off">
                            <div id="autocomplete-list" class="autocomplete-list"></div>
                        </div>
                        <div class="form-row">
                            <div class="form-group">
                                <label for="item-quantity">Quantity <span>*</span></label>
                                <input type="number" id="item-quantity" name="item_quantity" value="1" readonly required style="background-color: #e2e8f0; cursor: not-allowed;">
                                <div class="note-text">
                                    Note: Quantity is strictly 1 per record because individual animals may vary in price or weight.
                                </div>
                            </div>
                            
                            <div class="form-group">
                                <label for="item-weight">Weight (kg)</label>
                                <input type="number" id="item-weight" name="weight" placeholder="0.00" step="0.01" min="0">
                            </div>

                            <div class="form-group">
                                <label for="unit-display">Unit <span>*</span></label>
                                <input type="text" id="unit-display" value="<?php echo htmlspecialchars($default_unit_name); ?>" readonly>
                                <input type="hidden" id="unit" name="unit_id" value="<?php echo $default_unit_id; ?>">
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-group">
                                <label for="unit-cost">Cost (₱) <span>*</span></label>
                        ...
                        <div class="form-group">
                            <label for="item-desc">Description / Remarks</label>
                            <textarea id="item-desc" name="item_description" placeholder="Enter details like source farm, batch number, or health status" rows="3" maxlength="500"></textarea>
                        </div>
                    </div>
    ...
    <div id="confirm-all-modal" class="modal">
        <div class="modal-content" style="max-width: 450px;">
            <div class="modal-body confirm-content">
                ...
                <p style="color: #64748b;">This will confirm and lock <strong>ALL</strong> currently pending animal purchases.</p>
                <div class="warning-text">
                    ⚠️ <strong>Warning:</strong> This action cannot be undone. Please review all pending items before proceeding.
                </div>
            </div>
            <div class="modal-footer" style="justify-content: center; border-top: none; padding-top: 0; padding-bottom: 30px;">
    ...
    <script>
        ...
        function submitConfirmAll() {
            ...
            fetch('../purchase_confirmations/confirmAllAnimalPurchases.php', {
                method: 'POST'
            })
            .then(response => response.json())
            .then(data => {
        ...
        function fetchAutocomplete(searchTerm) {
            fetch(`../process/searchAnimals.php?term=${encodeURIComponent(searchTerm)}`)
                .then(response => response.json())
        ...
        function openAddModal() {
            ...
            document.getElementById('item-weight').value = ''; // RESET WEIGHT
            const today = new Date().toISOString().split('T')[0];
            document.getElementById('purchase-date').value = today;
        ...
        function viewItem(button) {
            ...
            const data = {
                ...
                weight: row.dataset.weight, // GET WEIGHT
        ...
        function populateEditForm(data) {
            ...
            document.getElementById('item-quantity').value = 1; // Strict
            document.getElementById('item-weight').value = data.weight; // SET WEIGHT
            ...
            if(data.building_id) {
                ...
                filterPens();
                const penSelect = document.getElementById('pen_id');
                if(data.pen_id) { penSelect.value = data.pen_id; }
            }
            hideAlert();
            document.getElementById('modal').classList.add('show');
            setTimeout(() => { initAutocomplete(); }, 100);
        ...
        function deleteItem(button) {
            ...
            const itemId = row.dataset.itemId;
            const itemName = row.dataset.itemName;
            if (confirm(`Are you sure you want to delete record for "${itemName}"? This action cannot be undone.`)) {
