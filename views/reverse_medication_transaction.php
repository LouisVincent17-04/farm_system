...
try {
    ...
    $time_stmt = $conn->query("SELECT TRANSACTION_DATE FROM treatment_transactions ORDER BY TRANSACTION_DATE DESC LIMIT 1");
    $latest_date = $time_stmt->fetchColumn();

    if ($latest_date) {
        // 2. Fetch ALL medication records matching that exact timestamp
        $sql = "SELECT 
                    tt.TT_ID,
                    tt.TRANSACTION_DATE,
                    tt.QUANTITY_USED,
                ...
                WHERE tt.TRANSACTION_DATE = :latest_date
...
<head>
    <meta charset="UTF-8">
    <title>Admin - Reverse Batch Medication</title>
    <style>
        :root { --dark: #0f172a; --dark-light: #1e293b; --red: #ef4444; --purple: #a855f7; --amber: #f59e0b; }
        ...
        .batch-header { padding: 1rem; background: rgba(168, 85, 247, 0.1); border-bottom: 1px solid #334155; display: flex; justify-content: space-between; align-items: center; }
...
<div class="container">
    <div class="card">
        <div class="header">
            <h1>
                <svg width="32" height="32" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"></path></svg>
                Reverse Batch Medication
            </h1>
            <p>Delete the most recent group of records and restore stock.</p>
        </div>
        ...
        <?php if (!empty($batch_records)): ?>
            ...
            <div class="batch-container">
                <div class="batch-header">
                    ...
                    <span style="font-size: 0.85rem; color: var(--amber);">
