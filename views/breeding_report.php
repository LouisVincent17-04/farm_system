...
checkAccess('breeding_reproduction_report');
...
try {
    ...
    // --- 2. BUILD SQL QUERY ---
    // Fetch items where ITEM_TYPE_ID = 6 (Breeding & Reproduction)
    $sql = "SELECT 
            i.ITEM_ID,
            i.ITEM_NAME,
            i.ITEM_DESCRIPTION,
    ...
    if ($debug_mode && $search_term) {
        $debug_info = "<div style='background: rgba(220, 38, 38, 0.1); color: #f87171; border: 1px solid rgba(220, 38, 38, 0.3); padding: 1.5rem; margin-bottom: 2rem; border-radius: 12px; font-family: var(--font-mono); font-size: 0.85rem;'>";
        $debug_info .= "<strong style='color: #ef4444;'><i class='fa-solid fa-bug'></i> DEBUG MODE ACTIVE</strong><br><br>";
        ...
        $debug_info .= "Results found: " . count($raw_data) . "<br><br>";
        $debug_info .= "<details><summary style='cursor: pointer; font-weight: bold; margin-bottom: 10px;'>Click to see SQL Query</summary><pre style='background: rgba(0,0,0,0.3); padding: 15px; overflow: auto; border-radius: 8px; margin-top: 10px;'>" . htmlspecialchars($sql) . "</pre></details>";
        
        // Check if any items contain search term regardless of ITEM_TYPE_ID
        ...
        $debug_info .= "<br><strong style='color: #fff;'>All items matching '" . htmlspecialchars($search_term) . "' (any type):</strong><br>";
        if ($test_results) {
    ...
    $items = [];
    $debug_info = "<div style='background: rgba(220, 38, 38, 0.1); color: #f87171; border: 1px solid rgba(220, 38, 38, 0.3); padding: 1.5rem; margin-bottom: 2rem; border-radius: 12px; font-family: var(--font-mono); font-size: 0.85rem;'>Error: " . htmlspecialchars($e->getMessage()) . "</div>";
...
<head>
    ...
    <title>Breeding Supplies Report</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body {
            ...
            padding-bottom: 60px;
            background-image:
                radial-gradient(ellipse 80% 50% at 50% -20%, rgba(225,29,72,0.06) 0%, transparent 60%),
        ...
        .stat-card.rose::before  { background: linear-gradient(135deg, rgba(225,29,72,0.04), transparent); }
        ...
        .stat-card.rose  { border-color: rgba(225,29,72,0.15); }
        ...
        .filter-badge {
            ...
            background: var(--rose); color: #fff; border-radius: 99px; padding: 0 6px;
        }
        .filter-toggle-btn {
        ...
        .form-control:focus {
            border-color: var(--border-active); box-shadow: 0 0 0 3px var(--rose-glow);
        ...
        .btn-primary:hover { background: #be123c; box-shadow: 0 0 16px var(--rose-glow); color: #fff; }
        ...
        .val-count { font-family: var(--font-mono); color: var(--rose); font-size: 0.85rem; font-weight: 600; }
...
<div class="container">
    ...
    <div class="top-bar">
        ...
        <span class="page-badge"><i class="fa-solid fa-dna"></i> Breeding & Reproduction</span>
    </div>

    <div class="page-header">
        <h1 class="page-title">Breeding <span>& Reproduction</span> Report</h1>
        <p class="page-subtitle">Log of breeding supplies, hormones, and reproductive tools.</p>
    </div>
    ...
    <div class="stats-grid">
        <div class="stat-card rose">
            <div class="stat-icon rose"><i class="fa-solid fa-microscope"></i></div>
            <div class="stat-val rose"><?= number_format($unique_items_count) ?></div>
            <div class="stat-lbl">Unique Items</div>
        </div>
        <div class="stat-card blue">
            <div class="stat-icon blue"><i class="fa-solid fa-boxes-stacked"></i></div>
            <div class="stat-val blue"><?= number_format($total_units) ?></div>
    ...
    <div class="filter-panel">
            ...
            <form method="GET" id="filterForm">
                <div class="filter-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="search" class="form-control" placeholder="e.g. Catheter, Hormones" value="<?= htmlspecialchars($search_term) ?>">
                    </div>
        ...
        <div class="filter-footer">
            <div class="filter-footer-left">
                <a href="breeding_report.php" class="btn btn-ghost btn-sm">
                        ...
                        <tr>
                            <td colspan="7">
                                <div class="empty-state">
                                    ...
                                    <p>No breeding supply records found matching your filters.</p>
                                </div>
                            </td>
                        </tr>
                    <?php else: ?>
...
<script>
    ...
    function exportPDF() {
        ...
        doc.setFontSize(16); doc.setTextColor(225, 29, 72); // Rose
        doc.text("Breeding Supplies Purchase Report", 14, 14);
        ...
        doc.save('Breeding_Supplies_Report.pdf');
    }
    ...
    function exportExcel() {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Breeding Supplies");
        XLSX.writeFile(wb, "Breeding_Report_" + new Date().toISOString().slice(0,10) + ".xlsx");
    }
    ...
    function exportCSV() {
        ...
        link.setAttribute("download", "Breeding_Report_" + new Date().toISOString().slice(0,10) + ".csv");
