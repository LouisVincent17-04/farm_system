...
// Default to the entire current year to ensure upcoming scheduled events are visible
...
<head>
    ...
    <style>
        :root {
            --primary: #f43f5e; --primary-dark: #e11d48;
        ...
        * { box-sizing: border-box; outline: none; -webkit-tap-highlight-color: transparent; }
        ...
        .back-link { display: inline-flex; align-items: center; gap: 6px; text-decoration: none; color: #94a3b8; font-weight: 600; font-size: 0.85rem; margin-top: 5px; transition: color 0.2s; }
        ...
        .group-header td { background: #0f172a; color: var(--primary); font-weight: 800; padding: 1rem 1.5rem; font-size: 0.95rem; border-bottom: 2px solid var(--border); text-transform: uppercase; letter-spacing: 0.5px; }
        ...
        /* Bulk action bar */
        #bulkActionBar { position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%) translateY(200%); width: 90%; max-width: 750px; background: rgba(30,41,59,0.95); backdrop-filter: blur(12px); padding: 1rem 1.5rem; border-radius: 16px; border: 1px solid rgba(255,255,255,0.1); display: flex; justify-content: space-between; align-items: center; z-index: 900; box-shadow: 0 20px 50px rgba(0,0,0,0.5); transition: transform 0.3s ease; }
        #bulkActionBar.active { transform: translateX(-50%) translateY(0); }
        .action-group { display: flex; gap: 8px; }
        .action-btn { padding: 8px 16px; border-radius: 8px; font-weight: 600; cursor: pointer; border: none; font-size: 0.85rem; transition: background 0.2s; }
        .btn-process    { background: var(--primary); color: white; box-shadow: 0 4px 10px rgba(244,63,94,0.3); }
        ...
        /* Pagination */
        .pagination-container { display: flex; justify-content: space-between; align-items: center; margin-top: 1.5rem; padding-top: 1rem; border-top: 1px solid var(--border); flex-wrap: wrap; gap: 1rem; }
        .page-info { color: var(--text-muted); font-size: 0.9rem; }
        .pagination { display: flex; gap: 5px; list-style: none; margin: 0; padding: 0; flex-wrap: wrap; justify-content: center; }
        ...
        .event-modal-card { background: var(--bg-card); border-radius: 16px; width: 100%; max-width: 960px; max-height: 92vh; border: 1px solid var(--border); overflow: hidden; display: flex; flex-direction: column; }
        ...
        .pen-pill:hover { border-color: var(--primary); background: rgba(244,63,94,0.06); }
        .pen-pill.selected { border-color: var(--primary); background: rgba(244,63,94,0.1); }
        .pen-pill input[type="checkbox"] {
            accent-color: var(--primary); width: 16px; height: 16px; cursor: pointer; flex-shrink: 0;
        }
        .pen-pill-name  { font-weight: 600; color: white; flex: 1; font-size: 0.9rem; }
        .pen-pill-count { font-size: 0.78rem; color: var(--text-muted); }
        .pen-pill.selected .pen-pill-count { color: #fda4af; }

        .pen-select-all-row {
            display: flex; align-items: center; justify-content: space-between;
            padding: 6px 2px; margin-bottom: 4px;
        }
        .pen-select-all-row label { font-size: 0.78rem; color: var(--text-muted); cursor: pointer; display: flex; align-items: center; gap: 6px; }
        .selected-summary {
            font-size: 0.8rem; color: #fda4af; font-weight: 600;
            padding: 6px 10px; background: rgba(244,63,94,0.1);
            border-radius: 6px; border: 1px solid rgba(244,63,94,0.2);
            display: none; margin-top: 8px; text-align: center;
        }
        .selected-summary.show { display: block; }

        /* Animal selection tree */
        ...
        @media (max-width: 768px) {
            ...
            .filter-actions { flex-direction: column; align-items: stretch; gap: 1rem; }
            ...
            .table tr.group-header { padding: 0.75rem 1rem; text-align: center; background: rgba(15,23,42,0.9); border: 1px solid var(--primary); border-radius: 8px; margin-top: 1.5rem; }
            .table tr.group-header td { display: block; text-align: center; color: var(--primary); padding: 0; border: none; }
            .table tr.group-header td::before { content: none; }
            .table td { padding: 0.5rem 0; border-bottom: 1px solid rgba(255,255,255,0.05); display: grid; grid-template-columns: 100px 1fr; align-items: start; text-align: left; }
            .table td::before { content: attr(data-label); font-weight: 700; color: var(--primary); font-size: 0.75rem; text-transform: uppercase; margin-right: 1rem; }
            .table td:last-child { border-bottom: none; }
            .table td:first-child { display: flex; flex-direction: row; justify-content: space-between; align-items: center; border-bottom: 1px dashed rgba(255,255,255,0.2); margin-bottom: 0.5rem; padding-bottom: 1rem; }
            .table td:first-child::before { content: "Select Event"; color: var(--text-muted); }
            .table tr.empty-row { display: block; text-align: center; padding: 3rem 1rem; }
            .table tr.empty-row td, .table tr.empty-row td:first-child { display: block; text-align: center; border: none; padding: 0; margin: 0; }
            .table tr.empty-row td::before, .table tr.empty-row td:first-child::before { content: none; display: none; }
            ...
            .event-modal-body { display: flex !important; flex-direction: column; overflow-y: auto; }
            .event-modal-left { border-right: none; border-bottom: 1px solid var(--border); overflow: visible; padding-bottom: 2rem; }
            .event-modal-right { overflow: visible; }
            .event-modal-footer { flex-direction: column; }
...
<div class="container">
        ...
        <div class="filter-grid">
            <div class="form-group span-2">
                ...
                <div style="position:relative;">
                    ...
                    <input type="text" name="search" class="form-control" placeholder="e.g. 1001 or Vitamin A"
                           value="<?= htmlspecialchars($search_query) ?>" style="padding-left:36px;">
                </div>
            </div>
            ...
            <div class="form-group span-2">
                ...
                <div class="flex gap-2">
                    <input type="text" name="date_from" class="form-control date-picker" value="<?= htmlspecialchars($date_from) ?>">
                    <input type="text" name="date_to"   class="form-control date-picker" value="<?= htmlspecialchars($date_to) ?>">
                </div>
            </div>
        </div>
                ...
                <?php else: ?>
                    ...
                    <tr class="<?= $isLocked ? 'row-locked' : '' ?>">
                        ...
                        <td data-label="Deadline">
                            <div style="color:<?= $isLocked?'inherit':'var(--primary)' ?>;font-weight:700;">
                        ...
                        <td data-label="Status">
                            ...
                            <?php elseif($ev['STATUS']==='Done'): ?>
                                <?php if($isLate): ?><span class="late-tag">Late (<?= $daysLate ?> days)</span>
                                <?php else: ?>      <span class="ontime-tag">On Time</span>
                                <?php endif; ?>
                            <?php endif; ?>
                            <?php if($ev['STATUS']!=='Pending' && !empty($ev['COMPLETED_AT'])): ?>
                                <div class="status-time" style="text-align:left;">
                                    <?= $isDone ? '✅' : '⛔' ?> <?= date('m/d/Y h:i A', strtotime($ev['COMPLETED_AT'])) ?>
                                </div>
                            <?php endif; ?>
                        </td>
                    </tr>
    ...
    <div class="pagination-container">
        <div class="page-info">
            Showing <strong><?= ($offset+1) ?></strong> to <strong><?= min($offset+$limit,$total_rows) ?></strong>
            of <strong><?= $total_rows ?></strong> events
        </div>
        <ul class="pagination">
            <li class="<?= $page_no<=1?'disabled':'' ?>">
                <a href="<?= getUrl($page_no-1) ?>">« Prev</a>
            </li>
            <?php
...
<div id="bulkActionBar">
    ...
    <div class="action-group">
        <button class="action-btn btn-process"     onclick="processSelectedEvents()">⚡ Process Tasks</button>
        <button class="action-btn btn-mark-done"   onclick="bulkUpdate('Done')">✓ Mark Done</button>
...
<div id="addModal" class="event-modal-overlay">
    <div class="event-modal-card">
        <div class="event-modal-header" style="padding:1.5rem;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;">
        ...
        <div class="event-modal-body">
            ...
            <div class="event-modal-left">
                <h4 style="margin-top:0;color:var(--primary);font-size:0.9rem;text-transform:uppercase;">
                    1. Select Targets
                </h4>
                ...
                <div class="form-group" style="margin-bottom:1rem;">
                    <label style="display:block;font-size:0.75rem;font-weight:700;text-transform:uppercase;color:var(--text-muted);margin-bottom:0.5rem;">
                        Building <span style="color:#ef4444">*</span>
                    </label>
                ...
                <div id="pen_picker_section" style="display:none;">
                    <label style="display:block;font-size:0.75rem;font-weight:700;text-transform:uppercase;color:var(--text-muted);margin-bottom:0.5rem;">
                        Pens <span style="color:#ef4444">*</span>
                        <span id="pen_count_badge" style="color:var(--primary);font-weight:800;margin-left:6px;"></span>
                    </label>
                    ...
                    <div class="pen-select-all-row">
                        ...
                        <span id="pen_selected_label" style="font-size:0.78rem;color:var(--text-muted);">0 selected</span>
                    </div>

                    <div id="pen_pill_list" class="pen-list"></div>

                    <div id="pen_animal_summary" class="selected-summary"></div>
                </div>

                <div id="selection_tree" class="selection-tree">
                    <div style="text-align:center;padding:1.5rem;color:var(--text-muted);font-size:0.9rem;border:1px dashed var(--border);border-radius:8px;">
            ...
            <form id="addEventForm" class="event-modal-right">
                <h4 style="margin-top:0;color:var(--primary);font-size:0.9rem;text-transform:uppercase;margin-bottom:1rem;">
                    2. Event Details
                </h4>
                ...
                <div class="form-group" style="margin-bottom:1rem;">
                    <label style="display:block;font-size:0.75rem;font-weight:700;text-transform:uppercase;color:var(--text-muted);margin-bottom:0.5rem;">Operation Type</label>
                ...
                <div class="form-group hidden" id="item_group" style="margin-bottom:1rem;">
                    <label id="item_label" style="display:block;font-size:0.75rem;font-weight:700;text-transform:uppercase;color:var(--text-muted);margin-bottom:0.5rem;">Item / Supply</label>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
                    <div class="form-group">
                        <label style="display:block;font-size:0.75rem;font-weight:700;text-transform:uppercase;color:var(--text-muted);margin-bottom:0.5rem;">Start Date</label>
                ...
                <div class="form-group" style="margin-top:10px;border-top:1px solid var(--border);padding-top:1rem;">
                    <label style="color:#94a3b8;font-weight:400;font-size:0.85rem;cursor:pointer;">
                ...
                <div class="form-group hidden" id="group_interval">
                    <label style="display:block;font-size:0.75rem;font-weight:700;text-transform:uppercase;color:var(--text-muted);margin-bottom:0.5rem;">Repeat Every (Days)</label>
    ...
    <div class="event-modal-card" style="max-width:500px;">
        ...
        <div class="event-modal-body" style="display:block;padding:1.5rem;">
            <div style="display:flex;gap:0;margin-bottom:1.5rem;border-bottom:2px solid var(--border);">
                <button class="tab-btn active" id="tab-btn-selection" onclick="switchTab('tab-selection')"
                        style="padding:10px;background:none;border:none;color:white;border-bottom:2px solid var(--primary);">By Selection</button>
                <button class="tab-btn" id="tab-btn-filter" onclick="switchTab('tab-filter')"
                        style="padding:10px;background:none;border:none;color:var(--text-muted);">By Criteria</button>
            </div>
            <div id="tab-selection" class="tab-content active">
                <p style="color:var(--text-muted);margin-bottom:1rem;">You have selected <strong id="modalSelectedCount" style="color:white;">0</strong> events to archive.</p>
                <button class="btn btn-danger" style="width:100%;" onclick="confirmArchive('selection')">Archive Selected</button>
            </div>
            <div id="tab-filter" class="tab-content" style="display:none;">
                <form id="filterArchiveForm">
                    <input type="hidden" name="action" value="bulk_delete">
                    <div class="form-group" style="margin-bottom:1rem;">
                        <label style="display:block;font-size:0.75rem;font-weight:700;text-transform:uppercase;color:var(--text-muted);margin-bottom:0.5rem;">Date Range</label>
                        <div class="flex gap-2">
                    ...
                    <div class="form-group" style="margin-bottom:1rem;">
                        <label style="display:block;font-size:0.75rem;font-weight:700;text-transform:uppercase;color:var(--text-muted);margin-bottom:0.5rem;">Location</label>
                    ...
                    <div class="form-group" style="margin-bottom:1rem;">
                        <label style="display:block;font-size:0.75rem;font-weight:700;text-transform:uppercase;color:var(--text-muted);margin-bottom:0.5rem;">Building</label>
                        <select name="del_bldg_id" id="del_bldg_id" class="form-control" disabled>
                            <option value="">All Buildings</option>
                        </select>
                    </div>
                    <button type="button" class="btn btn-danger" style="width:100%;" onclick="confirmArchive('filter')">Archive Matches</button>
                </form>
            </div>
        </div>
    </div>
...
<script>
/* ══════════════════════════════════════════════════════
   FLATPICKR INITIALIZATION (mm/dd/yyyy)
══════════════════════════════════════════════════════ */
document.addEventListener('DOMContentLoaded', () => {
    // Basic date pickers (Filters, Archive Modal)
    flatpickr(".date-picker", {
        dateFormat: "Y-m-d",      // Submits to PHP
        altInput: true,           // Creates dummy visual input
        altFormat: "m/d/Y",       // Displays as mm/dd/yyyy
        allowInput: true
    });

    // Date Time pickers (Add Event Modal)
    flatpickr(".datetime-picker", {
        enableTime: true,
        enableSeconds: true,
        dateFormat: "Y-m-d H:i:S", // Submits to PHP
        altInput: true,
        altFormat: "m/d/Y h:i K",  // Displays as mm/dd/yyyy hh:mm AM/PM
        allowInput: true
    });
});
...
function openModal(id) {
    ...
    if (id === 'addModal') {
        const now = new Date();
        // Set specific instance programmatically
        document.getElementById('start_date')._flatpickr.setDate(now);
    }
}
...
function openArchiveModal() {
    document.getElementById('modalSelectedCount').innerText = document.querySelectorAll('.row-chk:checked').length;
...
function resetAddModal() {
    ...
    document.getElementById('selection_tree').innerHTML =
        '<div style="text-align:center;padding:1.5rem;color:var(--text-muted);font-size:0.9rem;border:1px dashed var(--border);border-radius:8px;">Select a building to load pens.</div>';

    // Clear end date on reset
...
function switchTab(tabId) {
    ...
    document.querySelectorAll('.tab-btn').forEach(el => { el.style.borderBottom = 'none'; el.style.color = 'var(--text-muted)'; });
    document.getElementById(tabId).style.display = 'block';
    const btn = tabId === 'tab-selection' ? document.getElementById('tab-btn-selection') : document.getElementById('tab-btn-filter');
...
/* ══════════════════════════════════════════════════════
   STATE for add-modal
══════════════════════════════════════════════════════ */
let currentBuildingData = [];   // all pens + animals for selected building
let selectedPenIds      = new Set();  // which pens are currently checked
...
/* ══════════════════════════════════════════════════════
   CASCADE: Building → Pen pills (multi-select)
══════════════════════════════════════════════════════ */
async function handleModalBuildingChange(bldgId) {
    ...
    pillList.innerHTML = '';
    tree.innerHTML = '<div style="text-align:center;padding:1.5rem;color:var(--text-muted);font-size:0.9rem;border:1px dashed var(--border);border-radius:8px;">Select pens to load animals.</div>';
    selectedPenIds.clear();
    updatePenSummary();

    if (!bldgId) return;

    pillList.innerHTML = '<div style="color:var(--text-muted);font-size:0.85rem;padding:8px 0;">Loading pens…</div>';
    ...
    try {
        ...
        if (!currentBuildingData.length) {
            pillList.innerHTML = '<div style="color:var(--text-muted);font-size:0.85rem;padding:8px 0;">No pens found in this building.</div>';
            return;
        }

        // Count badge
        document.getElementById('pen_count_badge').textContent = `(${currentBuildingData.length} pens)`;

        // Render a pill checkbox for each pen
        currentBuildingData.forEach(pen => {
            ...
            pill.innerHTML = `
                ...
                <span class="pen-pill-count">${pen.animals.length} animal(s)</span>`;
        ...
        pillList.innerHTML = '<div style="color:var(--danger);font-size:0.85rem;">Error loading pens.</div>';
    }
}

/* ══════════════════════════════════════════════════════
   PEN TOGGLE  ── add or remove a pen's animals from the tree
══════════════════════════════════════════════════════ */
...
function updateSelectAllPenState() {
    ...
    document.getElementById('pen_selected_label').textContent =
        `${checked.length} of ${all.length} selected`;
}

function updatePenSummary() {
    const summary   = document.getElementById('pen_animal_summary');
    const totalAnimals = Array.from(document.querySelectorAll('.an-chk')).length;
    const checked      = Array.from(document.querySelectorAll('.an-chk:checked')).length;

    if (selectedPenIds.size === 0) {
        summary.classList.remove('show');
        return;
    }
    summary.classList.add('show');
    summary.textContent = `${selectedPenIds.size} pen(s) selected · ${totalAnimals} animal(s) available · ${checked} checked`;
}
...
function removePenFromTree(penId) {
    ...
    // If tree is empty, restore placeholder
    const tree = document.getElementById('selection_tree');
    if (!tree.querySelector('.pen-group')) {
        tree.innerHTML = '<div data-placeholder style="text-align:center;padding:1.5rem;color:var(--text-muted);font-size:0.9rem;border:1px dashed var(--border);border-radius:8px;">Check pens above to load their animals.</div>';
    }
    updatePenSummary();
}
...
async function loadItemsByLocation() {
    ...
    sel.innerHTML = '<option>Loading resources…</option>';
    ...
    try {
        ...
        if (!data.length) {
            sel.innerHTML = `<option value="">No ${type} available in this location</option>`;
        } else {
...
async function submitAddEvent() {
    ...
    // Collect ALL checked animal checkboxes across every pen in the tree
...
/* ══════════════════════════════════════════════════════
   PROCESS SELECTED EVENTS (route to correct page)
══════════════════════════════════════════════════════ */
function processSelectedEvents() {
    ...
    const pages = { Vaccination:'group_vaccination.php', Medication:'group_medication.php', Vitamins:'group_vitamins.php', Checkup:'group_checkup.php' };
...
function updateActionBar() {
    ...
    if (count > 0) {
        ...
        if (missingRecords) {
            ...
            btnProcess.style.boxShadow = '0 0 15px rgba(244,63,94,0.8)';
            btnProcess.style.transform = 'scale(1.05)';
        } else {
            btnDone.disabled  = false;
            btnDone.style.opacity = '1';
            btnDone.style.cursor  = 'pointer';
            btnDone.innerText = '✓ Mark Done';
            btnDone.setAttribute('data-locked', '0');
            btnProcess.style.boxShadow = '0 4px 10px rgba(244,63,94,0.3)';
            btnProcess.style.transform = 'scale(1)';
        }
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Anti-tamper on Mark Done
    const btnDone = document.querySelector('.btn-mark-done');
    if (btnDone) {
        new MutationObserver(() => {
            if (btnDone.getAttribute('data-locked') === '1' && !btnDone.disabled) {
                btnDone.disabled  = true;
                btnDone.innerText = '⛔ Tampering Blocked';
            }
...
async function bulkUpdate(status) {
    ...
    if (status === 'Done' && btn.getAttribute('data-locked') === '1') {
        showToast('Security: Cannot verify work records.', 'error'); return;
    }
