...
<head>
    ...
    <style>
        ...
        :root {
            ...
            --border-active:  rgba(245,158,11,0.5); /* Adapted to Amber for Feeds */
            --green:          #22c55e;
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .filter-badge {
            ...
            background: var(--gold); color: #000; border-radius: 99px; padding: 0 6px;
        }
        .filter-toggle-btn {
        ...
        .form-label.accent { color: var(--gold); }
        ...
        .btn-primary:hover { background: #d97706; box-shadow: 0 0 16px var(--gold-glow); }
        ...
        .val-weight { font-family: var(--font-mono); color: var(--gold);  font-size: 0.85rem; font-weight: 600; }

        /* ─── LEDGER BUTTON ─── */
        .btn-ledger {
            display: inline-flex; align-items: center; gap: 5px;
            padding: 5px 10px; background: var(--gold-dim);
            border: 1px solid rgba(245,158,11,0.25); color: var(--gold);
        ...
        .btn-ledger:hover {
            background: rgba(245,158,11,0.22); color: #fff; border-color: var(--gold);
            transform: translateY(-1px); box-shadow: 0 4px 12px var(--gold-glow);
        }
...
<div class="container">
    ...
    <div class="top-bar">
        ...
        <span class="page-badge"><i class="fa-solid fa-boxes-stacked"></i> Feed Supplies</span>
    </div>

    <div class="page-header">
        <h1 class="page-title">Feed <span>Inventory</span> Report</h1>
        <p class="page-subtitle">Monitor feed stock levels, valuations, and access individual ledgers.</p>
    </div>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card gold">
            <div class="stat-icon gold"><i class="fa-solid fa-weight-hanging"></i></div>
            <div class="stat-val gold"><?= number_format($total_weight, 2) ?></div>
    ...
    <div class="filter-panel">
        ...
        <div class="filter-body" id="filterBody">
            <form method="GET" id="filterForm">
                <div class="filter-grid">
                    ...
                    <div class="form-group">
                        <label class="form-label"><i class="fa-solid fa-magnifying-glass"></i> Search Feed</label>
                        <input type="text" name="search" class="form-control" placeholder="e.g. Starter" value="<?= htmlspecialchars($search_term) ?>">
                    </div>
        ...
        <div class="filter-footer">
            <div class="filter-footer-left">
                <a href="feed_report.php" class="btn btn-ghost btn-sm">
                        ...
                        <tr>
                            <td colspan="7">
                                <div class="empty-state">
                                    ...
                                    <p>No feed records found matching your filters.</p>
                                </div>
                            </td>
                        </tr>
                    <?php else: ?>
                        ...
                        ?>
                        <tr>
                            <td data-label="Feed Name" class="col-name"><?= htmlspecialchars($f['FEED_NAME']) ?></td>
                            <td data-label="Location"><?= htmlspecialchars($f['LOCATION_NAME'] ?? '-') ?></td>
                            <td data-label="Stock (KG)" class="text-right val-weight"><?= number_format($f['TOTAL_WEIGHT_KG'], 2) ?></td>
                            <td data-label="Total Cost" class="text-right val-money">₱<?= number_format($f['TOTAL_COST'], 2) ?></td>
                            <td data-label="Avg Cost/KG" class="text-right val-cost">₱<?= number_format($f['AVG_COST_PER_KG'], 2) ?></td>
                            <td data-label="Status"><span class="badge <?= $badgeClass ?>"><?= $f['STATUS_LABEL'] ?></span></td>
                            <td data-label="History" class="text-center">
                                <a href="viewFeedsLedger.php?id=<?= $f['FEED_ID'] ?>" class="btn-ledger">
...
<script>
    ...
    function exportPDF() {
        ...
        doc.setFontSize(16); doc.setTextColor(245, 158, 11); // Gold
        doc.text("Feed Inventory Report", 14, 14);
        ...
        doc.autoTable({
            head: [['Feed Name', 'Location', 'Stock (KG)', 'Total Cost (PHP)', 'Avg Cost/KG (PHP)', 'Status']],
            body: rows, startY: 26, 
    ...
    function exportExcel() {
        const excelData = records.map(r => ({
            ...
            'Avg Cost/KG (PHP)': parseFloat(r.AVG_COST_PER_KG), 
