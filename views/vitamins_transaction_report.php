...
try {
    ...
    // Filter: Search (Tag, Vitamin Name, Remarks)
    if ($search_term) {
        $sql .= " AND (ar.TAG_NO LIKE :search OR i.ITEM_NAME LIKE :search OR vt.REMARKS LIKE :search)";
...
<head>
    ...
    <style>
        ...
        th { 
            background: rgba(15, 23, 42, 0.9); color: #d9f99d; 
        ...
        <form method="GET">
            <div class="filter-grid">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="search" class="form-input" placeholder="e.g. Pig 01, Iron" value="<?= htmlspecialchars($search_term) ?>">
                </div>
                
                <div class="btn-group">
                    <button type="submit" class="btn btn-primary">Apply Filters</button>
                    <a href="vitamins_transaction_report.php" class="btn btn-outline">Reset</a>
                </div>
            </div>
        ...
        <table>
            ...
            <tbody>
                <?php if(empty($transactions)): ?>
                    <tr><td colspan="7" style="text-align:center; padding:3rem; color:#64748b;">No usage records found.</td></tr>
                <?php else: ?>
                    <?php foreach($transactions as $t): ?>
                    <tr>
                        <td style="color:#d9f99d; font-weight:bold;"><?= $t['TRANS_DATE'] ?></td>
                        <td>
                        ...
                        <td style="text-align:right; color:#4ade80;">₱<?= number_format($t['TOTAL_COST'], 2) ?></td>
...
<script>
    ...
    function exportPDF() {
        ...
        doc.autoTable({
            ...
            startY: 35,
            styles: { fontSize: 8 },
            headStyles: { fillColor: [101, 163, 13] } // Lime Header
        });
    ...
    function exportExcel() {
        ...
        XLSX.writeFile(wb, "Vitamin_Usage_Report_" + new Date().toISOString().slice(0,10) + ".xlsx");
    }

    // --- CSV Export ---
    function exportCSV() {
        let csvContent = "data:text/csv;charset=utf-8,";
        csvContent += "Date,Animal Tag,Vitamin,Dosage,Qty Used,Cost,Remarks\n";
        
        records.forEach(r => {
            const row = [
                r.TRANS_DATE, r.TAG_NO, r.ITEM_NAME, r.DOSAGE,
        ...
        link.setAttribute("download", "Vitamin_Usage_Report_" + new Date().toISOString().slice(0,10) + ".csv");
