...
try {
    ...
    $sql = "SELECT 
                ft.BATCH_ID,
                MAX(ft.TRANSACTION_DATE) as TRANSACTION_DATE,
                MAX(f.FEED_NAME) as FEED_NAME,
            ...
            ORDER BY MAX(ft.TRANSACTION_DATE) DESC, ft.BATCH_ID DESC 
            LIMIT 100"; // Reasonable limit to prevent massive payload
...
<head>
    ...
    <title>Reverse Feeding | FarmPro</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .form-select:focus { border-color: var(--amber); box-shadow: 0 0 0 3px rgba(245,158,11,0.15); background: var(--bg-hover); }
        ...
        .info-val.highlight { color: var(--amber); }
...
<div class="container">
    ...
    <div class="page-header">
        <h1 class="page-title"><i class="fa-solid fa-clock-rotate-left" style="color:var(--red);"></i> Admin <span>Undo</span> Feeding</h1>
        <p class="page-desc">Select a historical feeding batch to permanently delete its records and restore inventory.</p>
    </div>
        ...
        <div class="main-grid">
            ...
            <div class="control-panel">
                    ...
                    <select id="batchSelect" class="form-select" onchange="handleBatchSelection()">
                        ...
                        <?php foreach($batches as $b): ?>
                            <option value="<?= htmlspecialchars($b['BATCH_ID']) ?>">
                                Batch <?= htmlspecialchars($b['BATCH_ID']) ?> &bull; <?= date('M d', strtotime($b['TRANSACTION_DATE'])) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="alert-box">
                    ...
                    <div>
                        <strong>Warning:</strong> This action is permanent. Reversing a batch will completely delete its transaction records and re-add the used quantities back into your feed inventory.
                    </div>
                </div>
            </div>
            ...
            <div id="detailsPanel" class="details-panel">
                ...
                <div class="info-grid">
                    <div class="info-row">
                        <span class="info-lbl">Batch ID</span>
                        <span class="info-val highlight" id="lbl_batch_id">—</span>
                    </div>
                    <div class="info-row">
                    ...
                    <div class="info-row">
                        <span class="info-lbl">Commodity Used</span>
                        <span class="info-val" id="lbl_feed">—</span>
                    </div>
                    <div class="info-row">
                        <span class="info-lbl">Animals Affected</span>
                        <span class="info-val" id="lbl_count">—</span>
                    </div>
                    <div class="info-row">
                        <span class="info-lbl">Total Quantity</span>
                        <span class="info-val highlight" id="lbl_qty">—</span>
                    </div>
                    <div class="info-row" style="flex-direction: column; align-items: flex-start; gap: 8px;">
                        <span class="info-lbl">Remarks / Notes</span>
                        <span class="info-val" id="lbl_remarks" style="font-family: var(--font); font-size: 0.9rem; color: var(--text-secondary); font-weight: normal;">—</span>
                    </div>
                </div>
...
<script>
    ...
    function handleBatchSelection() {
        const selectedId = document.getElementById('batchSelect').value;
        ...
        const batch = batchData.find(b => String(b.BATCH_ID) === String(selectedId));

        if (batch) {
            // Format Date
            const d = new Date(batch.TRANSACTION_DATE);
            const dateStr = d.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute:'2-digit' });

            document.getElementById('lbl_batch_id').textContent = batch.BATCH_ID;
            document.getElementById('lbl_date').textContent = dateStr;
            document.getElementById('lbl_feed').textContent = batch.FEED_NAME || 'Unknown';
            document.getElementById('lbl_count').textContent = batch.animal_count + ' Heads';
            document.getElementById('lbl_qty').textContent = parseFloat(batch.total_kg).toFixed(2) + ' KG';
            document.getElementById('lbl_remarks').textContent = batch.remarks || 'No remarks provided.';
    ...
    function confirmReversal() {
        ...
        if(!confirm(`DANGER: Are you absolutely sure you want to delete Batch ${selectedId}?\n\nThis will permanently remove the transactions from the ledger and restore the feed stock.`)) {
            return;
        }
        ...
        .then(data => {
            if(data.success) {
                showToast("Reversal executed successfully.", "success");
