...
<head>
    ...
    <title>Group Vitamins & Supplements</title>
    <style>
        /* --- GLOBAL STYLES --- */
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .animal-card.selected { background: rgba(236, 72, 153, 0.2); border-color: #ec4899; }
        ...
        .custom-table input:focus, .custom-table select:focus { border-color: #ec4899; outline: none; }
...
<div class="container">
    <div class="main-grid">
        ...
        <div class="control-panel">
            <div class="panel-title">🧴 Group Vitamins</div>
            ...
            <form id="settingsForm">
                ...
                <label class="form-label" style="color:#f472b6;">STEP 2: Default Settings</label>
                ...
                <table class="custom-table">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 30%;">Supplement</th>
...
<script>
    ...
    <?php foreach($vits as $v): ?>
        inventory[<?= $v['SUPPLY_ID'] ?>] = {
            name: "<?= htmlspecialchars($v['SUPPLY_NAME']) ?>",
            stock: <?= $v['TOTAL_STOCK'] ?>,
            unit: "<?= $v['UNIT_ABBR'] ?>",
            unit_id: <?= $v['UNIT_ID'] ?>
        };
    ...
    function updateCalculations() {
        ...
        if (warnings.length > 0) {
            ...
            warningDiv.innerHTML = `<span style="color:#4ade80">Inventory levels sufficient.</span>`;
        } else {
        ...
        if (count > 0 && !hasError) {
            ...
            submitBtn.innerText = "Record Supplements";
        } else {
            submitBtn.disabled = true;
            submitBtn.innerText = hasError ? "Fix Stock Issues" : "Record Supplements";
        }
    }
    ...
    function submitBatch() {
        ...
        document.querySelectorAll('#vitamin-list tr[id^="row-"]').forEach(tr => {
            ...
            if(!itemId) {
                alert("Please select a supplement for all animals in the table.");
        ...
        if(validationError) {
            btn.disabled = false; btn.innerText = "Record Supplements";
            return;
        }
        ...
        fetch('../process/addBatchVitamins.php', {
            method: 'POST',
