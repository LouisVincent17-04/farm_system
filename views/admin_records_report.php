...
try {
    ...
    // --- 2. BUILD SQL QUERY ---
    // Fetch items where ITEM_TYPE_ID = 7 (Administration & Records)
    $sql = "SELECT 
            i.ITEM_ID,
            i.ITEM_NAME,
            i.ITEM_DESCRIPTION,
...
if ($location_id !== '' && $USER_LOCATION_ == 1000) $active_filters++; // Only count location filter if user actually chose it
...
<head>
    ...
    <title>Admin & Records Report</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body {
            ...
            padding-bottom: 60px;
            background-image:
                radial-gradient(ellipse 80% 50% at 50% -20%, rgba(148,163,184,0.06) 0%, transparent 60%),
                radial-gradient(ellipse 40% 30% at 85% 10%, rgba(100,116,139,0.04) 0%, transparent 50%);
        }
        ...
        .page-badge {
            ...
            color: var(--slate-light); background: var(--slate-dim); border: 1px solid rgba(148,163,184,0.2);
        ...
        .stat-icon.slate { background: var(--slate-dim); color: var(--slate-light); }
        ...
        .stat-val.slate-light { color: var(--slate-light); }
        .stat-val.slate-dark  { color: var(--slate); }
        ...
        .form-control:focus {
            border-color: var(--border-active); box-shadow: 0 0 0 3px var(--slate-glow);
        ...
        .btn-primary { background: var(--slate-dark); color: #fff; border-color: var(--slate-dark); }
        .btn-primary:hover { background: var(--slate); box-shadow: 0 0 16px var(--slate-glow); color: #000; }
        ...
        thead th {
            background: var(--bg-elevated); color: var(--slate-light);
        ...
        /* ─── VALUE CELLS ─── */
        .val-money { font-family: var(--font-mono); color: var(--slate); font-size: 0.8rem; font-weight: 600; }
        .val-cost  { font-family: var(--font-mono); color: var(--text-secondary); font-size: 0.8rem; }
        .val-count { font-family: var(--font-mono); color: var(--slate-light); font-size: 0.85rem; font-weight: 600; }
...
<div class="container">
    ...
    <div class="top-bar">
        ...
        <span class="page-badge"><i class="fa-solid fa-folder-open"></i> Administration & Records</span>
    </div>

    <div class="page-header">
        <h1 class="page-title">Admin <span>& Records</span> Report</h1>
        <p class="page-subtitle">Log of office supplies, logbooks, and administrative materials.</p>
    </div>

    <div class="stats-grid">
        <div class="stat-card slate">
            <div class="stat-icon slate"><i class="fa-solid fa-boxes-packing"></i></div>
            <div class="stat-val slate"><?= number_format($unique_items_count) ?></div>
            <div class="stat-lbl">Unique Items</div>
        </div>
        <div class="stat-card slate">
            <div class="stat-icon slate"><i class="fa-solid fa-layer-group"></i></div>
            <div class="stat-val slate-light"><?= number_format($total_units) ?></div>
            <div class="stat-lbl">Total Quantity</div>
        </div>
        <div class="stat-card slate">
            <div class="stat-icon slate"><i class="fa-solid fa-peso-sign"></i></div>
            <div class="stat-val slate-dark">₱<?= number_format($total_value, 2) ?></div>
    ...
    <div class="filter-panel">
            ...
            <form method="GET" id="filterForm">
                <div class="filter-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="search" class="form-control" placeholder="e.g. Logbook, Bond Paper" value="<?= htmlspecialchars($search_term) ?>">
                    </div>

                    <div class="form-group">
                        <label class="form-label"><i class="fa-solid fa-map-pin"></i> Location</label>
                        <select name="location" class="form-control <?= ($USER_LOCATION_ != 1000) ? 'locked' : '' ?>" <?= ($USER_LOCATION_ != 1000) ? 'aria-readonly="true"' : '' ?>>
        ...
        <div class="filter-footer">
            <div class="filter-footer-left">
                <a href="admin_records_report.php" class="btn btn-ghost btn-sm">
                        ...
                        <tr>
                            <td colspan="7">
                                <div class="empty-state">
                                    ...
                                    <p>No admin supply records found matching your filters.</p>
                                </div>
                            </td>
                        </tr>
                    <?php else: ?>
...
<script>
    ...
    function exportPDF() {
        ...
        doc.setFontSize(16); doc.setTextColor(100, 116, 139); // Slate
        doc.text("Admin & Records Supplies Report", 14, 14);
        ...
        doc.save('Admin_Supplies_Report.pdf');
    }
    ...
    function exportExcel() {
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Admin Supplies");
        XLSX.writeFile(wb, "Admin_Records_Report_" + new Date().toISOString().slice(0,10) + ".xlsx");
    }
    ...
    function exportCSV() {
        ...
        link.setAttribute("download", "Admin_Records_Report_" + new Date().toISOString().slice(0,10) + ".csv");
