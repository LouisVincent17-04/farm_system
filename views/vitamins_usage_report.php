...
<head>
    ...
    <title>Vitamins Usage & Accountancy Report</title>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .title { font-size: clamp(1.8rem, 4vw, 2.5rem); font-weight: 800; background: linear-gradient(135deg, #10b981, #059669); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom: 0.5rem; }
        .subtitle { color: #94a3b8; font-size: 1rem; margin-bottom: 0.5rem;}
        
        .location-badge { display: inline-flex; align-items: center; gap: 6px; margin-top: 10px; padding: 6px 16px; background: rgba(16, 185, 129, 0.15); border: 1px solid rgba(16, 185, 129, 0.3); color: #34d399; border-radius: 20px; font-weight: 700; font-size: 0.9rem; letter-spacing: 0.5px; text-transform: uppercase; }
        ...
        .c-stock { border-top: 4px solid #059669; } .c-stock .acc-val { color: #10b981; }
        ...
        .group-header { background: rgba(15, 23, 42, 0.8); padding: 1rem 1.5rem; font-weight: 700; color: #34d399; border-bottom: 1px solid #334155; font-size: 1.1rem; display: flex; align-items: center; gap: 10px; }
        ...
        .row-total { background: rgba(16, 185, 129, 0.1); font-weight: bold; border-top: 1px solid #334155 !important; }
...
<div class="container">
    ...
    <div class="header">
        <h1 class="title">Vitamins Usage & Ledger Report</h1>
        <p class="subtitle">Accountancy overview of vitamin purchases, administration, adjustments, and live balances.</p>
            ...
            <div class="filter-grid">
                ...
                <div class="form-group" style="grid-column: 1 / -1;">
                    <div class="btn-group">
                        ...
                        <a href="vitamins_usage_report.php" class="btn btn-outline">Reset</a>
                    </div>
                </div>
            </div>
    ...
    <?php if(empty($grouped_data) && $purchased_qty == 0 && empty($adjustments_data)): ?>
        <div class="empty-state">
            <h2 style="margin-bottom:10px;">No Vitamin Activity Found</h2>
    ...
    <?php else: ?>
        ...
        <?php foreach ($grouped_data as $header => $group): ?>
            <div class="group-container">
                <div class="group-header">
                    <i class="fa-solid fa-pills"></i> <?= htmlspecialchars($header) ?>
                </div>
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php foreach ($group['items'] as $item): ?>
                                <tr>
                                    <td data-label="Vitamin Name" style="font-weight:600; color:#fff;"><?= htmlspecialchars($item['ITEM_NAME']) ?></td>
                                    
                                    <td data-label="Purchased (IN)" class="t-right" style="color:#10b981;">
                                        <?= number_format($item['PURCHASED_QTY'], 2) ?>
                                    </td>
                                    
                                    <td data-label="Units Used (OUT)" class="t-right" style="color:#10b981; font-weight:bold;"><?= number_format($item['TOTAL_USED'], 2) ?></td>
                        ...
                        <tbody>
                            <?php foreach ($adjustments_data as $adj): ?>
                                <tr>
                                    ...
                                    <td data-label="Location" style="color:#34d399; font-weight:600;"><?= htmlspecialchars($adj['LOCATION_NAME']) ?></td>
                                    <td data-label="Vitamin Name" style="font-weight:600; color:#fff;"><?= htmlspecialchars($adj['ITEM_NAME']) ?></td>
                                    <td data-label="Type">
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php foreach ($purchases_by_item as $fName => $pData): ?>
                                ...
                                <tr>
                                    <td data-label="Location" style="color:#34d399; font-weight:600;"><?= htmlspecialchars($pData['location_name']) ?></td>
                                    <td data-label="Vitamin Name" style="font-weight:600; color:#fff;"><?= htmlspecialchars($pData['original_name']) ?></td>
        ...
        <div class="group-container" style="border-color: #10b981;">
            <div class="group-header" style="background: rgba(16, 185, 129, 0.2); color: #fff;">
                <i class="fa-solid fa-chart-pie"></i> GRAND TOTAL (Filtered Outwards)
            </div>
            <div class="table-wrap">
                <table>
                    <tbody>
                        <tr class="row-total" style="font-size: 1.1rem; background: rgba(16, 185, 129, 0.1);">
                            <td data-label="Summary" style="color: #34d399;">OVERALL ADMINISTERED</td>
                            <td data-label="Units Used (OUT)" class="t-right" style="color: #34d399;"><?= number_format($grand_total_used, 2) ?> units</td>
