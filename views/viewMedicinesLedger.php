...
if(isset($_GET['currpage']) && $_GET['currpage'] == 'med_report') {
    $redirect_url = "medicine_report.php";
    $back_button_text = "Back to Medicine Report";
} else {
    $redirect_url = "available_medicines.php";
    $back_button_text = "Back to Medicine Availability";
}

try {
    if (!$supply_id) throw new Exception("No Medicine ID provided.");

    // 1. Get Medicine Details
    $stmt = $conn->prepare("
        SELECT m.*, u.UNIT_NAME, u.UNIT_ABBR 
        FROM MEDICINES m 
    ...
    $unit_label = $medicine['UNIT_ABBR'] ?? 'units';

    // 2. Build Combined Ledger
    $ledger = [];

    // --- A. Fetch Usages (from treatment/medication transactions) ---
    try {
        $u_stmt = $conn->prepare("
            SELECT 
                tt.TRANSACTION_DATE AS raw_date,
                DATE_FORMAT(tt.TRANSACTION_DATE, '%m/%d/%Y %h:%i %p') AS txn_date_fmt,
                'Medical Treatment' AS txn_type,
                'Deduct' AS effect,
                tt.QUANTITY_USED AS qty,
        ...
        // Fallback in case table is named differently
    }
    ...
    try {
        $a_stmt = $conn->prepare("
            ...
            WHERE CATEGORY = 'medicine' AND REF_ID = ?
        ");
    ...
    try {
        ...
        ");
        $p_stmt->execute([$medicine['SUPPLY_NAME']]);
    ...
    foreach($ledger as $l) {
        ...
        } elseif ($l['txn_type'] === 'Medical Treatment') {
            $total_used += $l['qty'];
        } else {
            // Adjustments
...
<head>
    ...
    <title>Medicine Ledger | <?= htmlspecialchars($medicine['SUPPLY_NAME'] ?? 'Details') ?></title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        body {
            ...
            background-image: radial-gradient(ellipse 80% 50% at 50% -20%, rgba(244,63,94,0.05) 0%, transparent 60%);
        }
        ...
        /* ─── HEADER SECTION ─── */
        .supply-header-wrapper { margin-bottom: 2.5rem; }
        .supply-title { font-size: clamp(1.8rem, 4vw, 2.5rem); font-weight: 800; color: #fff; margin: 0 0 0.5rem 0; display: flex; align-items: center; flex-wrap: wrap; gap: 12px; letter-spacing: -0.02em;}
        .supply-title span.text-rose { background: linear-gradient(135deg, var(--rose), #be123c); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        .supply-subtitle { color: var(--text-secondary); font-size: 1rem; margin: 0; }
        ...
        .c-usage    { color: var(--pink); } 
        .c-adjust   { color: var(--rose); } 
        ...
        .type-usage { background: var(--pink-dim); color: var(--pink); border: 1px solid rgba(244,114,182,0.3); }
...
<div class="container">
    ...
    <?php if(isset($error)): ?>
        <div class="error-box">
            ...
            <p style="color: var(--text-muted); margin-top: 0.5rem;">The requested medicine profile could not be loaded.</p>
        </div>
    <?php else: ?>

        <div class="supply-header-wrapper">
            <h1 class="supply-title">
                <span class="text-rose"><?= htmlspecialchars($medicine['SUPPLY_NAME']) ?></span>
                <?php if(!empty($medicine['EXPIRATION_DATE']) && $medicine['EXPIRATION_DATE'] != '0000-00-00'): ?>
                    <span class="exp-badge">
                        <i class="fa-regular fa-calendar-xmark"></i> Exp: <?= date('M d, Y', strtotime($medicine['EXPIRATION_DATE'])) ?>
                    </span>
                <?php endif; ?>
            </h1>
        ...
        <div class="stats-grid">
            <div class="stat-card stat-purple">
                ...
                <div class="stat-val c-purchase"><?= number_format($total_purchased, 2) ?> <span><?= htmlspecialchars($unit_label) ?></span></div>
            </div>
            
            <div class="stat-card stat-pink">
                <div class="stat-lbl">Total Consumed</div>
                <div class="stat-val c-usage"><?= number_format($total_used, 2) ?> <span><?= htmlspecialchars($unit_label) ?></span></div>
            </div>

            <div class="stat-card stat-rose">
                <div class="stat-lbl">Net Adjustments</div>
                <div class="stat-val c-adjust"><?= ($net_adjustments > 0 ? '+' : '') . number_format($net_adjustments, 2) ?> <span><?= htmlspecialchars($unit_label) ?></span></div>
            </div>
            ...
            <div class="stat-card stat-emerald" style="background: rgba(16,185,129,0.05);">
                ...
                <div class="stat-val c-stock"><?= number_format($medicine['TOTAL_STOCK'], 2) ?> <span><?= htmlspecialchars($unit_label) ?></span></div>
            </div>
        </div>
                        ...
                        <?php if(empty($ledger)): ?>
                            <tr>
                                <td colspan="4" class="empty-state">
                                    ...
                                    No transaction history found for this medicine.
                                </td>
                            </tr>
                        <?php else: ?>
                            <?php foreach($ledger as $row): 
                                ...
                                if ($row['txn_type'] == 'Medical Treatment') { $badgeClass = 'type-usage'; $icon = 'fa-syringe'; }
