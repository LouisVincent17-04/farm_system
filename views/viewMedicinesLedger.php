...
if (!$supply_id) {
    header("Location: available_medicines.php");
    exit;
}
...
try {
    // 1. Fetch Medicine Info (Current Stock)
    $stmt = $conn->prepare("SELECT * FROM MEDICINES WHERE SUPPLY_ID = ?");
    $stmt->execute([$supply_id]);
    $item = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$item) throw new Exception("Medicine ID not found.");

    // 2. Build Query
    // We UNION 'ITEMS' (Purchases) and 'TREATMENT_TRANSACTIONS' (Usage)
    ...
    // Base Params for the Union
    // Param 1: Medicine Name (for Items table - Stock In)
    // Param 2: Supply ID (for Treatment table - Stock Out)
    ...
    $sql = "
        SELECT * FROM (
            ...
            FROM ITEMS i
            WHERE i.ITEM_NAME = ? 
            AND i.ITEM_TYPE_ID = 1 -- Ensure it's a Medicine

            UNION ALL

            -- STOCK OUT (From TREATMENT_TRANSACTIONS table)
            SELECT 
                t.TRANSACTION_DATE as T_DATE,
                'USAGE' as T_TYPE,
                CONCAT('Animal Tag: ', a.TAG_NO) as T_REF,
                (t.QUANTITY_USED * -1) as QTY_CHANGE, -- Negative to show deduction
                0 as COST_PER_UNIT,
                COALESCE(t.REMARKS, t.DOSAGE) as T_REMARKS
            FROM TREATMENT_TRANSACTIONS t
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-filter {
            background: #8b5cf6; color: white; border: none; padding: 8px 20px;
        ...
        .ledger-table th { 
            background: #0f172a; color: #a78bfa; padding: 15px 20px;
            text-align: left; text-transform: uppercase; font-size: 0.85rem; border-bottom: 2px solid #334155;
        }
...
<div class="container">
    <a href="available_medicines.php" class="btn-back">← Back to Medicines</a>
    ...
    <div class="header-card">
        <div class="title">
            ...
            <div style="color: #94a3b8;">Item ID: MED-<?= str_pad($item['SUPPLY_ID'], 3, '0', STR_PAD_LEFT) ?></div>
        </div>
        <div class="stock-box">
            <div style="color: #a78bfa; font-size: 0.8rem; text-transform: uppercase;">Current Stock</div>
    ...
    <form method="GET" class="filter-bar">
        ...
        <div class="filter-group" style="flex-grow:1;">
            <input type="text" name="search" class="form-control" style="width:100%;" placeholder="Search reference, animal tag, or remarks..." value="<?= htmlspecialchars($search_term) ?>">
        </div>

        <button type="submit" class="btn-filter">Apply</button>
        <a href="viewMedicineLedger.php?id=<?= $supply_id ?>" class="btn-reset">Reset</a>
    </form>
        ...
        <table class="ledger-table">
            ...
            <tbody>
                <?php if(count($transactions) > 0): ?>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr><td colspan="5" style="text-align:center; padding:3rem; color:#64748b;">No transaction history found.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
