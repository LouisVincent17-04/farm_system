<?php
// views/costing_vitamins_supplies.php
...
if (isset($_GET['action'])) {
    ...
    if ($action === 'get_pen_history' && isset($_GET['pen_id'])) {
        ...
        $query = "SELECT 
                    ar.TAG_NO,
                    vst.TRANSACTION_DATE, 
                    vs.SUPPLY_NAME, 
                    vst.QUANTITY_USED, 
                    vst.TOTAL_COST as CALCULATED_COST, 
                    vst.REMARKS 
                  ...
                  WHERE ar.PEN_ID = ?
                  ORDER BY vst.TRANSACTION_DATE DESC";
        ...
        if ($stmt->rowCount() > 0) {
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $html .= "<tr>
                            ...
                            <td data-label=\"Supply Name\" style=\"font-weight:700; color:#fff;\">{$row['SUPPLY_NAME']}</td>
            ...
            $html = "<tr><td colspan='6' class='empty-state'><i class='fa-solid fa-ghost' style='font-size:2rem; display:block; margin-bottom:1rem; opacity:0.5;'></i> No vitamin/supplement history found for this pen.</td></tr>";
        }
    ...
    if ($action === 'get_history' && isset($_GET['animal_id'])) {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $html = "<tr><td colspan='5' class='empty-state'><i class='fa-solid fa-ghost' style='font-size:2rem; display:block; margin-bottom:1rem; opacity:0.5;'></i> No vitamin/supplement history found for this animal.</td></tr>";
        }
...
<head>
    ...
    <title>Vitamins Cost History | FarmPro</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tag-badge { background: var(--amber-dim); color: var(--amber); padding: 4px 10px; border-radius: 6px; font-size: 0.85rem; font-weight: 700; font-family: var(--font-mono); border: 1px solid rgba(245,158,11,0.3); display: inline-flex; align-items: center; gap: 4px;}
...
<div class="container">
    ...
    <div class="top-bar">
        ...
        <span class="page-badge"><i class="fa-solid fa-flask"></i> Nutrition Analytics</span>
    </div>

    <header class="page-header">
        <div class="header-info">
            <h1>Vitamins Cost <span>History</span></h1>
            <p>Analyze historical supplement usage and associated nutritional costs.</p>
        </div>
    </header>
        ...
        <div class="workspace-panel">
            <div id="resultsArea" class="table-section">
                ...
                <div class="section-header">
                    <div>
                        <div class="section-title"><i class="fa-solid fa-list-ul"></i> Supplement Transactions</div>
                ...
                <div class="total-box">
                    <span class="lbl">Total Supply Cost</span>
...
<script>
    ...
    async function fetchData(params) {
        try {
            // Point to THIS file
            const response = await fetch(`costing_vitamins_supplies.php?${params}`);
