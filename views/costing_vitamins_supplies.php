...
if (isset($_GET['action'])) {
    ...
    if ($action === 'get_pen_history' && isset($_GET['pen_id'])) {
        ...
        if ($stmt->rowCount() > 0) {
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $html .= "<tr>
                            <td><span style='background: rgba(245, 158, 11, 0.2); padding: 4px 8px; border-radius: 4px; font-weight: 600; color: #fbbf24; white-space: nowrap;'>{$row['TAG_NO']}</span></td>
                            <td style='white-space: nowrap;'>{$date}</td>
                            <td>{$row['SUPPLY_NAME']}</td>
                            <td style='white-space: nowrap;'>{$row['QUANTITY_USED']} units</td>
                            <td style='color: #fbbf24; font-weight: bold; white-space: nowrap;'>₱ {$cost}</td>
                            <td>{$row['REMARKS']}</td>
                          </tr>";
            }
        } else {
            $html = "<tr><td colspan='6' style='text-align:center; padding: 2rem;'>No vitamin/supplement history found for this pen.</td></tr>";
        }
    ...
    if ($action === 'get_history' && isset($_GET['animal_id'])) {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $html = "<tr><td colspan='5' style='text-align:center; padding: 2rem;'>No vitamin/supplement history found for this animal.</td></tr>";
        }
...
<head>
    ...
    <title>Vitamins & Supplements Cost History</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .page-title {
            ...
            background: linear-gradient(135deg, #fbbf24, #d97706); /* Amber/Orange for Vitamins */
            -webkit-background-clip: text; -webkit-text-fill-color: transparent;
        }

        .filter-container {
            background: rgba(30, 41, 59, 0.6);
            border: 1px solid rgba(245, 158, 11, 0.2); /* Orange Border */
            border-radius: 16px;
            padding: 2rem;
        ...
        .form-select:focus { border-color: #fbbf24; }
        ...
        .scroll-hint-text {
            ...
            color: #fbbf24; /* Amber for Vitamins Theme */
...
<div class="container">
    ...
    <header class="page-header">
        <h1 class="page-title">Vitamins & Supplements Cost</h1>
    ...
    <div id="resultsArea" class="results-section">
        <div class="table-header-box">
            ...
            <div style="display: flex; gap: 1rem; align-items: center; flex-wrap: wrap;">
                ...
                <div id="selectedTagDisplay" style="color: #fbbf24; font-weight: bold;"></div>
            </div>
        </div>
        ...
        <div class="total-box">
            Total Supply Cost: <span style="color: #fbbf24; font-weight: bold; margin-left: 10px;">₱ <span id="grandTotal">0.00</span></span>
        </div>
    </div>
...
<script>
    ...
    function triggerMobileScrollHint() {
        ...
        // Trigger condition: <= 1100px width and hasn't been shown yet
        if (window.innerWidth <= 1100 && !hasShownScrollHint) {
