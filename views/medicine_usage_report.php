<?php
// views/medicine_usage_report.php
ob_start();
error_reporting(0);
...
try {
    // --- A. FETCH CONFIRMED PURCHASES (INWARDS) ---
    $purch_params = [];
    $purch_where = "";
    ...
    if ($q_date_from && $q_date_to) { 
        $where .= " AND DATE(tt.TRANSACTION_DATE) BETWEEN :df AND :dt";
    ...
    $sql = "SELECT 
                ...
                $select_tag,
                COALESCE(m.SUPPLY_NAME, 'Deleted Medicine') AS ITEM_NAME,
                SUM(tt.QUANTITY_USED) as TOTAL_USED,
                SUM(tt.TOTAL_COST) as TOTAL_COST,
                MIN(DATE(tt.TRANSACTION_DATE)) as MIN_DATE,
                MAX(DATE(tt.TRANSACTION_DATE)) as MAX_DATE
            ...
            $where
            GROUP BY $group_by_clause, m.SUPPLY_NAME
            ORDER BY loc.LOCATION_NAME, bld.BUILDING_NAME, p.PEN_NAME, m.SUPPLY_NAME";
    ...
    $adj_params = [];
    $adj_where = " WHERE ia.CATEGORY = 'medicine' ";
    
    if ($date_from && $date_to) {
        $adj_where .= " AND DATE(ia.TRANSACTION_DATE) BETWEEN :df AND :dt ";
        $adj_params[':df'] = $date_from;
        $adj_params[':dt'] = $date_to;
    }
    
    if ($f_loc) {
        $adj_where .= " AND m.LOCATION_ID = :aloc";
    ...
    $adj_sql = "SELECT 
                ...
                LEFT JOIN medicines m ON ia.REF_ID = m.SUPPLY_ID
    ...
    $stock_sql = "SELECT SUM(TOTAL_STOCK) as current_stock FROM medicines $stock_where";
...
<head>
    ...
    <title>Medicine Usage & Accountancy Report</title>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .title { font-size: clamp(1.8rem, 4vw, 2.5rem); font-weight: 800; background: linear-gradient(135deg, #e11d48, #be123c); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom: 0.5rem; }
        .subtitle { color: #94a3b8; font-size: 1rem; margin-bottom: 0.5rem;}
        
        .location-badge { display: inline-flex; align-items: center; gap: 6px; margin-top: 10px; padding: 6px 16px; background: rgba(225, 29, 72, 0.15); border: 1px solid rgba(225, 29, 72, 0.3); color: #fb7185; border-radius: 20px; font-weight: 700; font-size: 0.9rem; letter-spacing: 0.5px; text-transform: uppercase; }

        /* SUMMARY CARDS */
        .acc-summary-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1.5rem; margin-bottom: 2rem; }
        .acc-card { background: rgba(30, 41, 59, 0.6); border: 1px solid rgba(255,255,255,0.1); border-radius: 16px; padding: 1.5rem; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); }
        ...
        .c-stock { border-top: 4px solid #e11d48; } .c-stock .acc-val { color: #fb7185; }
        ...
        .btn { padding: 10px 20px; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; font-size: 0.9rem; transition: transform 0.1s; }
        .btn:active { transform: scale(0.98); }
        .btn-primary { background: #e11d48; color: white; }
        .btn-outline { background: transparent; border: 1px solid #475569; color: #cbd5e1; text-decoration: none;}
        .btn-pdf { background: #ef4444; color: white; } .btn-excel { background: #3b82f6; color: white; } .btn-csv { background: #f59e0b; color: white; }
        ...
        .group-header { background: rgba(15, 23, 42, 0.8); padding: 1rem 1.5rem; font-weight: 700; color: #fda4af; border-bottom: 1px solid #334155; font-size: 1.1rem; display: flex; align-items: center; gap: 10px; }
        ...
        .row-total { background: rgba(225, 29, 72, 0.1); font-weight: bold; border-top: 1px solid #334155 !important; }
...
<div class="container">
    ...
    <div class="header">
        <h1 class="title">Medicine Usage & Ledger Report</h1>
        <p class="subtitle">Accountancy overview of medicine purchases, usage, adjustments, and live balances.</p>
    ...
    <div class="acc-summary-grid">
        <div class="acc-card c-inwards">
            ...
            <div class="acc-val"><?= number_format($purchased_qty, 2) ?> units</div>
            <div class="acc-sub">₱ <?= number_format($purchased_cost, 2) ?></div>
        </div>
        
        <div class="acc-card c-outwards">
            <h4>📤 Total Administered (OUT)</h4>
            <div class="acc-val"><?= number_format($grand_total_used, 2) ?> units</div>
        ...
        <div class="acc-card c-variance">
            ...
            ?>
            <div class="acc-val"><?= number_format($var_qty, 2) ?> units</div>
        ...
        <div class="acc-card c-adjust">
            ...
            ?>
            <div class="acc-val" style="color:<?= $adj_color ?>;"><?= $adj_sign . number_format($net_adjustments, 2) ?> units</div>
        ...
        <div class="acc-card c-adj-net">
            ...
            ?>
            <div class="acc-val"><?= number_format($expected_net, 2) ?> units</div>
            <div class="acc-sub">IN - OUT + ADJ</div>
        </div>

        <div class="acc-card c-stock">
            <h4>📦 Live Warehouse Stock</h4>
            <div class="acc-val"><?= number_format($current_stock, 2) ?> units</div>
            ...
            <div class="filter-grid">
                ...
                <div class="form-group" style="grid-column: 1 / -1;">
                    <div class="btn-group">
                        ...
                        <a href="medicine_usage_report.php" class="btn btn-outline">Reset</a>
                    </div>
                </div>
            </div>
    ...
    <?php if(empty($grouped_data) && $purchased_qty == 0 && empty($adjustments_data)): ?>
        <div class="empty-state">
            <h2 style="margin-bottom:10px;">No Medicine Activity Found</h2>
    ...
    <?php else: ?>
        ...
        <?php foreach ($grouped_data as $header => $group): ?>
            <div class="group-container">
                <div class="group-header">
                    <i class="fa-solid fa-capsules"></i> <?= htmlspecialchars($header) ?>
                </div>
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php foreach ($group['items'] as $item): ?>
                                <tr>
                                    <td data-label="Medicine Name" style="font-weight:600; color:#fff;"><?= htmlspecialchars($item['ITEM_NAME']) ?></td>
                                    
                                    <td data-label="Purchased (IN)" class="t-right" style="color:#10b981;">
                                        <?= number_format($item['PURCHASED_QTY'], 2) ?>
                                    </td>
                                    
                                    <td data-label="Units Used (OUT)" class="t-right" style="color:#e11d48; font-weight:bold;"><?= number_format($item['TOTAL_USED'], 2) ?></td>
                            ...
                            <tr class="row-total">
                                ...
                                <td data-label="Units Used (OUT)" class="t-right"><?= number_format($group['sub_used'], 2) ?></td>
        ...
        <?php if(!empty($adjustments_data)): ?>
            <div class="group-container" style="border-color: #a855f7;">
                <div class="group-header adjustments">
                    <i class="fa-solid fa-wrench"></i> INVENTORY ADJUSTMENTS (MEDICINES)
                </div>
                <div class="table-wrap">
                    <table>
                        <thead>
                        ...
                        <tbody>
                            <?php foreach ($adjustments_data as $adj): ?>
                                <tr>
                                    ...
                                    <td data-label="Location" style="color:#fb7185; font-weight:600;"><?= htmlspecialchars($adj['LOCATION_NAME']) ?></td>
                                    <td data-label="Medicine Name" style="font-weight:600; color:#fff;"><?= htmlspecialchars($adj['ITEM_NAME']) ?></td>
                                    <td data-label="Type">
                                    ...
                                    <td data-label="Units" class="t-right" style="font-weight:bold; color: <?= strtolower($adj['ADJUSTMENT_TYPE']) == 'add' ? '#34d399' : '#f87171' ?>;">
                                        <?= strtolower($adj['ADJUSTMENT_TYPE']) == 'add' ? '+' : '-' ?><?= number_format($adj['QUANTITY'], 2) ?>
                                    </td>
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php foreach ($purchases_by_item as $fName => $pData): ?>
                                ...
                                <tr>
                                    <td data-label="Location" style="color:#fb7185; font-weight:600;"><?= htmlspecialchars($pData['location_name']) ?></td>
                                    <td data-label="Medicine Name" style="font-weight:600; color:#fff;"><?= htmlspecialchars($pData['original_name']) ?></td>
                                    <td data-label="Purchased (IN)" class="t-right" style="color:#10b981; font-weight:bold;">
                                        <?= number_format($pData['qty'], 2) ?>
                                    </td>
                                    <td data-label="Units Used (OUT)" class="t-right" style="color:#64748b;">0.00</td>
        ...
        <div class="group-container" style="border-color: #e11d48;">
            <div class="group-header" style="background: rgba(225, 29, 72, 0.2); color: #fff;">
                <i class="fa-solid fa-chart-pie"></i> GRAND TOTAL (Filtered Outwards)
            </div>
            <div class="table-wrap">
                <table>
                    <tbody>
                        <tr class="row-total" style="font-size: 1.1rem; background: rgba(225, 29, 72, 0.1);">
                            <td data-label="Summary" style="color: #fb7185;">OVERALL ADMINISTERED</td>
                            <td data-label="Units Used (OUT)" class="t-right" style="color: #fb7185;"><?= number_format($grand_total_used, 2) ?> units</td>
...
<script>
    ...
    function generateExportData() {
        ...
        if(adjustmentsData && adjustmentsData.length > 0) {
            ...
            rows.push(['Date', 'Location & Medicine Name', 'Type', 'Units Adjusted', 'Reason']);
    ...
    function exportPDF() {
        ...
        doc.text("Medicine Usage & Ledger Report", 14, 15);
        ...
        doc.save('Medicine_Ledger_Report.pdf');
    }
    ...
    function exportExcel() {
        ...
        XLSX.writeFile(wb, `Medicine_Ledger_Report_${filenameDate}.xlsx`);
    }
    ...
    function exportCSV() {
        ...
        link.setAttribute("download", `Medicine_Ledger_Report_${filenameDate}.csv`);
